...
function ReisenApp() {
  ...
  const addName = () => {
    ...
    saveNames([n, ...names]); // newest trip goes first, matching "neueste Reise links"
    setNewName("");
  };
  ...
  const renameManagedTrip = async (oldName, newName) => {
    ...
    // Update every flight tagged with the old name to keep them in sync.
  ...
  // The names list (top management section) directly defines the display
  // order for badges and detail cards below — no separate order list to
  // keep in sync. Trips not yet in "names" (shouldn't normally happen, but
  // guards against stale data) are appended at the end.
  ...
  return (
    <div style={{minHeight:"100vh",background:"#241805",color:"#e8f4fd",fontFamily:"-apple-system,BlinkMacSystemFont,sans-serif",paddingBottom:40}}>
      ...
      {/* Verwaltung: eigene Liste zum Anlegen/Umbenennen/Sortieren der Reisen,
          unabhängig von den Badges/Karten weiter unten. */}
      <div style={{padding:"14px 16px 0"}}>
        <div style={{fontSize:11,color:"rgba(232,244,253,0.4)",textTransform:"uppercase",letterSpacing:0.5,marginBottom:8}}>Reisen verwalten</div>
      ...
      {/* Badges: eine pro Reise, Name+Nummer+Datum, horizontal scrollbar,
          neueste Reise links. */}
      <div style={{padding:"16px 0 0 16px",display:"flex",gap:10,overflowX:"auto",WebkitOverflowScrolling:"touch"}}>
        ...
        {trips.map((trip, tripIdx) => (
          <div key={trip.name} style={{flexShrink:0,width:260,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,overflow:"hidden"}}>
            {/* Badge: name, running number, date range */}
            ...
            {/* Details: flight list + summary, directly below this trip's own badge */}
                ...
                <div style={{display:"flex",flexDirection:"column",gap:6}}>
                  {trip.flights.map((f, idx) => (
                    <div key={f.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:13}}>
                      ...
                      <span style={{color:"rgba(232,244,253,0.6)",flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{f.name} · {f.date}</span>
