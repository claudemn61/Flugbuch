...
function ReisenApp() {
    ...
    (async () => {
      try {
        ...
        const PALETTE = { white:"var(--c-text, #e8f4fd)", navy:"#040e20", red:"#f87171", yellow:"#fde047",
          cyan:"#7dd3fc", bordeaux:"#e0304a", orange:"var(--c-accent-reisen, #f5a623)", green:"#4ade80", violet:"#c4b5fd" };
        ...
        const CATS = [
          ...
          ["error","--c-error",1], ["rating","--c-rating",1], ["accentReisen","--c-accent-reisen",1],
        ];
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--c-bg, #241805)",color:"var(--c-text, #e8f4fd)",fontFamily:"-apple-system,BlinkMacSystemFont,sans-serif",paddingBottom:40}}>
      ...
      <div style={{padding:"14px 16px 0"}}>
          ...
          {names.map((n, idx) => (
            <div key={n} style={{display:"flex",alignItems:"center",gap:8,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:10,padding:"6px 8px"}}>
              <input value={n} onChange={e=>renameManagedTrip(n, e.target.value)}
                style={{flex:1,background:"transparent",border:"none",color:"var(--c-text, #e8f4fd)",fontSize:14,padding:"4px 6px",minWidth:0}} />
              <button onClick={()=>moveName(n, -1)} disabled={idx===0}
                style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,width:26,height:26,color:idx===0?"rgba(232,244,253,0.2)":"var(--c-text, #e8f4fd)",fontSize:12,cursor:idx===0?"default":"pointer",flexShrink:0}}>▲</button>
              <button onClick={()=>moveName(n, 1)} disabled={idx===names.length-1}
                style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,width:26,height:26,color:idx===names.length-1?"rgba(232,244,253,0.2)":"var(--c-text, #e8f4fd)",fontSize:12,cursor:idx===names.length-1?"default":"pointer",flexShrink:0}}>▼</button>
            </div>
          ))}
        </div>
        <div style={{display:"flex",gap:8,marginTop:8}}>
          <input value={newName} onChange={e=>setNewName(e.target.value)}
            ...
            style={{flex:1,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,padding:"9px 12px",color:"var(--c-text, #e8f4fd)",fontSize:14,boxSizing:"border-box"}} />
          <button onClick={addName}
            style={{background:"rgba(245,166,35,0.18)",border:"1px solid rgba(245,166,35,0.4)",borderRadius:10,padding:"9px 16px",color:"var(--c-accent-reisen, #f5a623)",fontSize:13,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap"}}>
            + Anlegen
          </button>
        </div>
        </>)}
      ...
      <div style={{padding:"16px 0 0 16px",display:"flex",gap:10,overflowX:"auto",WebkitOverflowScrolling:"touch"}}>
        ...
        {trips.map((trip, tripIdx) => (
          <div key={trip.name} style={{flexShrink:0,width:360,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,overflow:"hidden"}}>
            <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:10,background:"rgba(245,166,35,0.08)",borderBottom:"1px solid rgba(245,166,35,0.15)"}}>
              <span style={{fontSize:12,fontWeight:700,color:"var(--c-accent-reisen, #f5a623)",background:"rgba(245,166,35,0.18)",borderRadius:20,width:26,height:26,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                {trips.length - tripIdx}
              </span>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:15,fontWeight:700,color:"var(--c-text, #e8f4fd)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{trip.name}</div>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:6}}>
                  {trip.flights.map((f, idx) => {
                    ...
                    <div key={f.id}
                      ...
                      {distVal>0 && <span style={{color:"rgba(232,244,253,0.35)",fontSize:11,flexShrink:0}}>{distVal}km</span>}
                      <span style={{color:"var(--c-accent-reisen, #f5a623)",fontWeight:700,flexShrink:0}}>{fmtHM(f.durationSec||0)}</span>
                    </div>
                    );
                  })}
                </div>
