...
function aggregate(flights, keyFn) {
  ...
  const rows = [...groups.values()].map(g => {
    ...
    return { name: g.key, count: fl.length, totalSec, maxSec, totalDist, maxDist, maxAlt, first, last, startSites, endSites, flights: fl };
  });
  return rows;
}
...
function StatTable({ table, sortOptions }) {
  ...
  return (
    <div style={{margin:"12px 16px 0",display:"flex",flexDirection:"column",gap:8}}>
      ...
      {openDetail && <FlightListOverlay row={openDetail} onClose={()=>setOpenDetail(null)} />}
    </div>
  );
}
...
function FlightListOverlay({ row, onClose }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"#210710",zIndex:300,overflowY:"auto"}}>
      ...
      <div style={{padding:"0 16px 24px"}}>
        {sorted.map(f => (
          <div key={f.id} style={{padding:"11px 0",borderBottom:"1px solid rgba(255,255,255,0.06)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <div style={{textAlign:"right",flexShrink:0,marginLeft:8}}>
              <div style={{fontSize:13,fontWeight:600,color:"#f87171"}}>{f.durationStr||fmtHM(f.durationSec||0)}</div>
