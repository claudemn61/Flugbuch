...
function FlightMap({ flight }) {
  ...
  const draw = (canvas) => {
    ...
    (async () => {
      ...
      // Use the exact same lon/lat -> canvas-pixel transform as the track
      // drawing below, so tiles and the flight path always line up. Convert
      // each tile's corner coordinates back to lon/lat, but do it via each
      // tile's own top-left corner computed independently (not by walking
      // tile-by-tile), which keeps all tiles internally consistent with
      // each other because they all derive from the same xi/yi -> lon/lat
      // formula rather than accumulating separate rounding per draw call.
      const sc=Math.min((W-24)/(maxLon-minLon||0.001),(H-24)/(maxLat-minLat||0.001));
      const offX=(W-(maxLon-minLon)*sc)/2, offY=(H-(maxLat-minLat)*sc)/2;
      const tx=lon=>offX+(lon-minLon)*sc, ty=lat=>H-offY-(lat-minLat)*sc;
      ...
      let anyLoaded = false;
      // Compute one shared tile pixel size from the zoom level's known tile
      // geometry, rather than re-deriving width/height per tile from
      // lon/lat round-trips (which can differ by a fraction of a pixel per
      // tile and reopen the seam/gap problem). All tiles at a given zoom
      // are the same size in tile-space, so they must be the same size on
      // screen too, whatever that shared size works out to be.
      const refA = tileToLonLat(xMin, yMin, zoom);
      const refB = tileToLonLat(xMin+1, yMin+1, zoom);
      const tileW = Math.abs(tx(refB.lon) - tx(refA.lon));
      const tileH = Math.abs(ty(refB.lat) - ty(refA.lat));
      tiles.forEach(({ xi, yi, img }) => {
        if (!img) return;
        anyLoaded = true;
        const corner = tileToLonLat(xi, yi, zoom);
        const px = tx(corner.lon), py = ty(corner.lat);
        ctx.drawImage(img, Math.round(px), Math.round(py), Math.ceil(tileW)+1, Math.ceil(tileH)+1);
      });
      if (!anyLoaded) {
        ctx.fillStyle = "#0d1b2a"; ctx.fillRect(0,0,W,H);
      } else {
