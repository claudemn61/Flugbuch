...
// by the manually-saved order (reisen:order) — highest number = first in
...
function computeReiseLabels(flights, reiseOrder) {
  ...
  ordered.forEach((name, idx) => {
    ...
    const sorted = [...byTrip.get(name)].sort((a,b) => parseDateToTs(a.date) - parseDateToTs(b.date));
...
function FlugbuchApp() {
  ...
  const [reisenOrder, setReisenOrder] = useState([]);
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get("reisen:names");
      ...
      try {
        ...
        if (r2) setReisenOrder(JSON.parse(r2.value) || []);
      } catch {}
    })();
  }, []);
  ...
  const reiseLabels = useMemo(() => computeReiseLabels(flights, reisenOrder), [flights, reisenOrder]);
