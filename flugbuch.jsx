...
function parseIGC(text) {
  ...
  const track = [];
  let date = "", pilot = "", glider = "", passagier = "";
  ...
  return { track, date, pilot, glider, passagier };
}
...
function analyzeIGC(track) {
  ...
  // The UI displays durationStr/startTime/endTime (not the raw seconds),
  // so those need to exist too, not just durationSec — otherwise "Dauer",
  // "Startzeit" and "Landezeit" show as blank even though the underlying
  // number is right there in durationSec.
  const fmtClock = (sec) => {
    const h = Math.floor(sec/3600)%24, m = Math.floor((sec%3600)/60);
    return `${String(h).padStart(2,"0")}:${String(m).padStart(2,"0")}`;
  };
  const durH = Math.floor(durationSec/3600), durM = Math.round((durationSec%3600)/60);
  ...
  // H.Diff. and Ø Speed are never auto-computed elsewhere for flights with
  // a real track (saveComputedField deliberately skips those, on the
  // assumption the track itself is the more accurate source) — so they
  // need to be derived here instead, or they'd just stay blank forever.
  const hDiff = Math.abs(startAlt - endAlt);
  const distKm = +(totalDist/1000).toFixed(2);
  const kmh = durationSec > 0 ? +(distKm / (durationSec/3600)).toFixed(1) : 0;
  return { maxAlt, minAlt, startAlt, endAlt, startPt, endPt, durationSec, durationStr, startTime, endTime,
    totalDist: distKm, thermalCount: thermals.length, maxClimb, maxSinkRate, totalGain: Math.round(totalGain), hDiff, kmh };
}
    ...
    return (
      <div onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}
        ...
        <div style={{padding:"0 16px"}}>
          ...
          {/* Stats grid — capped at 9 tiles for a symmetric 3x3 layout;
              if that would be exceeded, H.Diff. is dropped first since
              Start/Land. müM already convey the same information. */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:14}}>
            {(() => {
              const tiles = [
                ...
                fl.customFields?.hGew ? ["📈", fl.customFields.hGew+" m", "Höhengewinn"] : null,
              ].filter(Boolean);
              const shown = tiles.length > 9 ? tiles.filter(t => t[2] !== "H.Diff.") : tiles;
              return shown.map(([ic,v,l])=>(
...
function FlugbuchApp() {
  ...
  const processIGCFiles = useCallback(async (igcFiles) => {
    ...
    for (let i=0; i<igcFiles.length; i++) {
      ...
      const text = await file.text();
      const { track, date, pilot, glider, passagier } = parseIGC(text);
      ...
      if (existing) {
        ...
        if (!(cf.hDiff||"").trim() && igcData.hDiff) cf.hDiff = String(igcData.hDiff);
        if (!(cf.kmh||"").trim() && igcData.kmh) cf.kmh = String(igcData.kmh);
        ...
        const updated = {
          ...
          durationSec: existing.durationSec || igcData.durationSec,
          durationStr: (existing.durationStr||"").trim() ? existing.durationStr : igcData.durationStr,
          totalDist: existing.totalDist || igcData.totalDist,
        ...
        const newF = { id:`igc_${baseName}_${Date.now()}`, name:baseName, pdfOnly:false,
          ...
          customFields:{passagier:passagier||"",landung:"",
            ...
            kmh: igcData.kmh ? String(igcData.kmh) : "",
