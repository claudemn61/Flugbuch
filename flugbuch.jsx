...
function FlightMap({ flight }) {
  ...
  const draw = (canvas, isFullscreenCanvas) => {
    ...
    if (!track.length && (!sP||!eP)) {
      ...
      ctx.fillStyle = "var(--c-bg, #040e20)"; ctx.fillRect(0,0,W,H);
  ...
  return (
    <>
      <div style={{position:"relative"}} onClick={()=>{ if (hasMap) setIsFullscreen(true); }}>
        <canvas ref={canvasRef} style={{width:"100%",height:140,background:"var(--c-bg, #040e20)",borderRadius:10,display:"block",cursor:hasMap?"pointer":"default"}} />
        {hasMap && (
        ...
        <div style={{marginTop:6,display:"flex",gap:6,alignItems:"center"}}>
          ...
          <div style={{display:"flex",background:"rgba(255,255,255,0.05)",borderRadius:8,padding:2}}>
            <button onClick={()=>setGpsvColorBy("altitude")}
              style={{background:gpsvColorBy==="altitude"?"rgba(125,211,252,0.25)":"transparent",border:"none",borderRadius:6,padding:"5px 8px",color:gpsvColorBy==="altitude"?"var(--c-accent-flug, #7dd3fc)":"rgba(232,244,253,0.5)",fontSize:10,fontWeight:700,cursor:"pointer"}}>
              Höhe
            </button>
            <button onClick={()=>setGpsvColorBy("climb")}
              style={{background:gpsvColorBy==="climb"?"rgba(125,211,252,0.25)":"transparent",border:"none",borderRadius:6,padding:"5px 8px",color:gpsvColorBy==="climb"?"var(--c-accent-flug, #7dd3fc)":"rgba(232,244,253,0.5)",fontSize:10,fontWeight:700,cursor:"pointer"}}>
              Steigen/Sinken
            </button>
          </div>
        </div>
...
function FlightProfile({ flight }) {
  ...
  return (
    <div style={{marginBottom:14}}>
      <div style={{fontSize:10,fontWeight:700,color:"var(--c-accent-flug, #7dd3fc)",letterSpacing:1.5,textTransform:"uppercase",marginBottom:6}}>Höhenprofil</div>
      <div style={{borderRadius:14,overflow:"hidden",border:"1px solid rgba(100,180,255,0.12)",background:"var(--c-bg, #040e20)"}}>
    ...
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.7)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:"#0f2033",borderRadius:20,padding:20,width:"100%",maxWidth:420,maxHeight:"80vh",overflowY:"auto",border:"1px solid rgba(100,180,255,0.15)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          ...
          <button onClick={onClose} style={{background:"none",border:"none",color:"var(--c-accent-flug, #7dd3fc)",fontSize:20,cursor:"pointer"}}>✕</button>
        </div>
        {defs.map(f=>(
            ...
            {f.formula ? (
              ...
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                <input value={f.name} onChange={e=>update(f.id,"name",e.target.value)} placeholder="Feldname"
                  style={{flex:1,background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"6px 10px",color:"var(--c-text, #e8f4fd)",fontSize:13}} />
                <select value={f.type} onChange={e=>update(f.id,"type",e.target.value)}
                  style={{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"6px 8px",color:"var(--c-text, #e8f4fd)",fontSize:12}}>
        ...
        <div style={{marginTop:12,marginBottom:12}}>
          ...
          <div style={{display:"flex",gap:8}}>
            {["text","number","date"].map(t=>(
              <button key={t} onClick={()=>add(t)} style={{flex:1,background:"rgba(100,180,255,0.1)",border:"1px solid rgba(100,180,255,0.2)",borderRadius:10,padding:"8px 4px",color:"var(--c-accent-flug, #7dd3fc)",fontSize:12,cursor:"pointer"}}>
...
function SeasonDash({ flights, onBack, pdfData }) {
  ...
  const S = {
    wrap:{padding:"0 16px 24px",background:"var(--c-bg, #040e20)",minHeight:"100vh",color:"var(--c-text, #e8f4fd)",fontFamily:"-apple-system,BlinkMacSystemFont,sans-serif"},
    yearRow:{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"},
    yrBtn:(a)=>({background:a?"rgba(14,165,233,0.3)":"rgba(255,255,255,0.05)",border:a?"1px solid rgba(14,165,233,0.5)":"1px solid rgba(255,255,255,0.08)",borderRadius:20,padding:"6px 14px",color:a?"var(--c-accent-flug, #7dd3fc)":"rgba(232,244,253,0.5)",fontSize:13,cursor:"pointer",fontWeight:a?600:400}),
    ...
    bigNum:{fontSize:26,fontWeight:800,color:"var(--c-accent-flug, #7dd3fc)",letterSpacing:-1},
  ...
  return (
    <div style={S.wrap}>
      <div style={{display:"flex",alignItems:"center",gap:10,padding:"calc(20px + env(safe-area-inset-top, 0px)) 0 14px",borderBottom:"1px solid rgba(100,180,255,0.1)",marginBottom:16}}>
        ...
        <button onClick={onBack} style={{background:"none",border:"none",color:"var(--c-accent-flug, #7dd3fc)",fontSize:22,cursor:"pointer",padding:0}}>‹</button>
        <div>
          <div style={{fontSize:11,fontWeight:600,color:"var(--c-accent-flug, #7dd3fc)",letterSpacing:1.5,textTransform:"uppercase"}}>Saison-Übersicht</div>
          ...
          <div onClick={e=>e.stopPropagation()}
            ...
            {years.slice(4).map(y=>(
              <div key={y} onClick={()=>{setYr(y);setShowMoreYears(false);}}
                style={{padding:"10px 12px",borderRadius:10,fontSize:15,cursor:"pointer",color:y===yr?"var(--c-accent-flug, #7dd3fc)":"var(--c-text, #e8f4fd)",background:y===yr?"rgba(14,165,233,0.15)":"transparent",marginBottom:2}}>
                {y}
              </div>
            ))}
          </div>
...
function FlightRow({ f, isLongest, onClick, sortId, selectMode, isSelected, onToggleSelect, reiseLabel }) {
  ...
  return (
    <div onClick={selectMode ? ()=>onToggleSelect(f.id) : onClick}
      ...
      {selectMode && (
        <div style={{marginRight:10,flexShrink:0,width:20,height:20,borderRadius:6,border:`2px solid ${isSelected?"var(--c-accent-flug, #7dd3fc)":"rgba(232,244,253,0.3)"}`,background:isSelected?"var(--c-accent-flug, #7dd3fc)":"transparent",display:"flex",alignItems:"center",justifyContent:"center"}}>
      ...
      <div style={{textAlign:"right",flexShrink:0,marginLeft:8}}>
        <div style={{fontSize:13,fontWeight:600,color:"var(--c-accent-flug, #7dd3fc)",display:"flex",alignItems:"center",justifyContent:"flex-end",gap:4}}>
    ...
    <div style={{position:"relative"}}>
      <div style={{position:"relative"}}>
        <input value={filterText} onChange={e=>setFilterText(e.target.value)} onFocus={()=>setAdvOpen(true)} placeholder="🔍 Suchen…"
          style={{width:"100%",background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,padding:"8px 34px 8px 12px",color:"var(--c-text, #e8f4fd)",fontSize:13,boxSizing:"border-box"}} />
        {filterText && (
      ...
      {advOpen && (
        <div style={{position:"absolute",top:"calc(100% + 8px)",left:0,width:"min(92vw, 420px)",zIndex:50,background:"#0f1f36",boxShadow:"0 12px 32px rgba(0,0,0,0.5)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:12,padding:10}}>
            ...
            {rows.map((row, idx) => {
              ...
              return (
                <div key={idx} style={{display:"flex",gap:6,alignItems:"center"}}>
                  <span style={{fontSize:10,fontWeight:700,color:"var(--c-accent-flug, #7dd3fc)",minWidth:34,textAlign:"center",flexShrink:0}}>
                  ...
                  <select value={row.field}
                    ...
                    style={{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:8,padding:"5px 4px",color:"var(--c-text, #e8f4fd)",fontSize:12,minWidth:0}}>
                  ...
                  {(() => {
                    ...
                    return (
                      <select value={row.op || (isNumeric ? "=" : ":")} onChange={e=>updateRow(idx,{op:e.target.value})}
                        style={{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:8,padding:"5px 2px",color:"var(--c-text, #e8f4fd)",fontSize:12,width:isNumeric?68:44,flexShrink:0}}>
                  ...
                  <input value={row.value==="*"?"":row.value} onChange={e=>updateRow(idx,{value:e.target.value})}
                    ...
                    disabled={row.value==="*"}
                    style={{flex:1,minWidth:0,background:row.value==="*"?"rgba(255,255,255,0.03)":"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:8,padding:"5px 8px",color:"var(--c-text, #e8f4fd)",fontSize:12}} />
                  {row.op==="between" && (
                    <input value={row.value2||""} onChange={e=>updateRow(idx,{value2:e.target.value})} placeholder="bis…"
                      style={{flex:1,minWidth:0,background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:8,padding:"5px 8px",color:"var(--c-text, #e8f4fd)",fontSize:12}} />
                  )}
                  {fieldDef?.anyOption && (
                    <button onClick={()=>updateRow(idx,{value: row.value==="*" ? "" : "*"})}
                      title="Beliebiger Passagier (Biplace-Flüge)"
                      style={{background:row.value==="*"?"rgba(125,211,252,0.25)":"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:8,padding:"5px 8px",color:row.value==="*"?"var(--c-accent-flug, #7dd3fc)":"rgba(232,244,253,0.6)",fontSize:11,fontWeight:700,cursor:"pointer",flexShrink:0,whiteSpace:"nowrap"}}>
                      beliebig
                    </button>
                  )}
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:8}}>
            <button onClick={addRow} style={{background:"rgba(125,211,252,0.12)",border:"1px solid rgba(125,211,252,0.3)",borderRadius:8,padding:"5px 10px",color:"var(--c-accent-flug, #7dd3fc)",fontSize:11,fontWeight:700,cursor:"pointer"}}>+ Zeile</button>
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              {rows.length>1 && (
                <div style={{display:"flex",background:"rgba(255,255,255,0.06)",borderRadius:8,padding:2}}>
                  <button onClick={()=>applyRows(rows,"AND")} style={{background:combine==="AND"?"rgba(125,211,252,0.25)":"transparent",border:"none",borderRadius:6,padding:"4px 10px",color:combine==="AND"?"var(--c-accent-flug, #7dd3fc)":"rgba(232,244,253,0.5)",fontSize:11,fontWeight:700,cursor:"pointer"}}>UND</button>
                  <button onClick={()=>applyRows(rows,"OR")} style={{background:combine==="OR"?"rgba(125,211,252,0.25)":"transparent",border:"none",borderRadius:6,padding:"4px 10px",color:combine==="OR"?"var(--c-accent-flug, #7dd3fc)":"rgba(232,244,253,0.5)",fontSize:11,fontWeight:700,cursor:"pointer"}}>ODER</button>
                </div>
              )}
...
function CoordEdit({lat, lon, alt, color, onSave}) {
  ...
  const iStyle = {width:"100%",background:"rgba(255,255,255,0.08)",border:"1px solid rgba(125,211,252,0.4)",borderRadius:6,padding:"3px 6px",color:"var(--c-text, #e8f4fd)",fontSize:11,fontFamily:"monospace",boxSizing:"border-box",marginBottom:3};
  if (editing) {
    return (
      <div>
        ...
        <input value={al} onChange={e=>setAl(e.target.value)} placeholder="müM" style={iStyle}
          onKeyDown={e=>{ if(e.key==="Enter"){e.preventDefault();commit();} }} />
        <button onClick={commit} style={{width:"100%",background:"rgba(125,211,252,0.15)",border:"1px solid rgba(125,211,252,0.3)",borderRadius:6,padding:"3px",color:"var(--c-accent-flug, #7dd3fc)",fontSize:10,cursor:"pointer"}}>✓ Speichern</button>
      </div>
    );
  }
  return (
  ...
  if (editing) {
    return (
      <input value={val} onChange={e=>setVal(e.target.value)} onBlur={commit} autoFocus
        onKeyDown={e=>{ if(e.key==="Enter"){e.preventDefault();commit();} }}
        style={{fontSize:22,fontWeight:800,marginBottom:4,background:"rgba(255,255,255,0.08)",border:"1px solid rgba(125,211,252,0.4)",borderRadius:8,padding:"2px 8px",color:"var(--c-text, #e8f4fd)",width:"100%",boxSizing:"border-box"}} />
    );
  }
  return (
...
function StaticField({label, value, unit}) {
  return (
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
      ...
      <span style={{fontSize:13,fontWeight:500,color:value?"var(--c-text, #e8f4fd)":"rgba(232,244,253,0.25)",textAlign:"right"}}>
      ...
      {editing ? (
        multiline
          ? <textarea value={val} onChange={e=>setVal(e.target.value)} onBlur={commit} autoFocus
              style={{flex:1,background:"rgba(255,255,255,0.08)",border:"1px solid rgba(125,211,252,0.4)",borderRadius:8,padding:"4px 8px",color:"var(--c-text, #e8f4fd)",fontSize:13,resize:"vertical",minHeight:48}} />
          : <input value={val} onChange={e=>setVal(e.target.value)} onBlur={commit} autoFocus
              data-inline-field
              onKeyDown={e=>{
                if(e.key==="Enter"){
              ...
              style={{flex:1,background:"rgba(255,255,255,0.08)",border:"1px solid rgba(125,211,252,0.4)",borderRadius:8,padding:"4px 8px",color:"var(--c-text, #e8f4fd)",fontSize:13,textAlign:"right"}} />
      ) : (
        <span data-inline-field-trigger onClick={()=>{setVal(value||"");setEditing(true);}}
          style={{fontSize:13,fontWeight:500,color:value?"var(--c-text, #e8f4fd)":"rgba(232,244,253,0.25)",cursor:"pointer",minWidth:60,textAlign:"right"}}>
          {value||(unit?"— "+unit:"—")}
        </span>
      )}
    </div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {coordChoice.candidates.map((c,i)=>(
                <button key={i} onClick={()=>{ onSave(coordChoice.name, c); setCoordChoice(null); }}
                  style={{textAlign:"left",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:10,padding:"10px 12px",color:"var(--c-text, #e8f4fd)",cursor:"pointer"}}>
  ...
  return (
    <div data-inline-row style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
      ...
      <select value={value||""} onChange={e=>onSave(e.target.value)}
        style={{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:8,padding:"4px 8px",color:value?"var(--c-text, #e8f4fd)":"rgba(232,244,253,0.4)",fontSize:13,textAlign:"right",maxWidth:180}}>
  ...
  if (!editing) {
    return (
      <div data-inline-row onClick={()=>setEditing(true)}
        ...
        <span style={{fontSize:13,color:value?"var(--c-text, #e8f4fd)":"rgba(232,244,253,0.4)"}}>{value || "—"}</span>
      </div>
    );
  }
...
function DetailContent({ fl, flights, customFieldDefs, setFlights, setSelected, setView, setInlinePassagier, setEditData, saveFlight, showFieldEditor, setShowFieldEditor, handleSaveFields, confirmDelete, setConfirmDelete, hideBackButton, isWide, returnTo }) {
    ...
    return (
      <div onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}
        style={{maxWidth:isWide?720:480,margin:"0 auto",padding:"0 0 32px",background:"var(--c-bg, #040e20)",minHeight:"100vh",color:"var(--c-text, #e8f4fd)",fontFamily:"-apple-system,BlinkMacSystemFont,sans-serif"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"calc(16px + env(safe-area-inset-top, 0px)) 16px 10px"}}>
          {!hideBackButton && <button onClick={()=>{ if (returnTo) { window.location.href = returnTo; } else { setView("list"); } }} style={{background:"none",border:"none",color:"var(--c-accent-flug, #7dd3fc)",fontSize:22,cursor:"pointer"}}>←</button>}
        ...
        <div style={{padding:"0 16px"}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:2}}>
            <span style={{fontSize:11,color:"var(--c-accent-flug, #7dd3fc)"}}>{fl.date}</span>
          ...
          <div style={{background:"rgba(255,255,255,0.04)",borderRadius:14,padding:"13px 15px",marginBottom:14,border:"1px solid rgba(255,255,255,0.06)"}}>
            ...
            {notesEditing ? (
              <textarea value={notesVal} onChange={e=>setNotesVal(e.target.value)} onBlur={commitNotes} autoFocus
                style={{width:"100%",background:"rgba(255,255,255,0.08)",border:"1px solid rgba(125,211,252,0.4)",borderRadius:8,padding:"8px 10px",color:"var(--c-text, #e8f4fd)",fontSize:13,resize:"vertical",minHeight:60,textAlign:"left",boxSizing:"border-box"}} />
            ) : (
              <div onClick={()=>{setNotesVal(fl.notes||"");setNotesEditing(true);}}
                style={{width:"100%",fontSize:13,fontWeight:500,color:fl.notes?"var(--c-text, #e8f4fd)":"rgba(232,244,253,0.25)",cursor:"pointer",textAlign:"left",whiteSpace:"pre-wrap",minHeight:20,lineHeight:1.5}}>
            ...
            {tileConfig.map((key, i) => {
              ...
              return (
                <div key={i} onClick={()=>setTilePickerIdx(i)}
                  ...
                  <div style={{fontSize:14,fontWeight:800,color:"var(--c-accent-flug, #7dd3fc)"}}>{opt.get(fl)}</div>
          ...
          <div id="flugdaten-section" style={{background:"rgba(255,255,255,0.04)",borderRadius:14,padding:"13px 15px",marginBottom:11,border:"1px solid rgba(255,255,255,0.06)"}}>
            <div style={{fontSize:10,fontWeight:700,color:"var(--c-accent-flug, #7dd3fc)",letterSpacing:1.5,textTransform:"uppercase",marginBottom:9}}>Flugdaten</div>
            ...
            <div onClick={e=>e.stopPropagation()}
              ...
              <div style={{display:"flex",gap:10}}>
                <button onClick={()=>setConfirmDelete(null)}
                  style={{flex:1,background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:10,padding:"10px",color:"var(--c-text, #e8f4fd)",fontSize:14,cursor:"pointer"}}>Abbrechen</button>
...
function SidebarList({ flights, selectedId, onSelect, longestId }) {
  ...
  return (
    <div style={{width:340,minWidth:340,height:"100vh",overflowY:"auto",borderRight:"1px solid rgba(255,255,255,0.08)",background:"var(--c-bg, #040e20)",fontFamily:"-apple-system,BlinkMacSystemFont,sans-serif"}}>
      <div style={{padding:"calc(14px + env(safe-area-inset-top, 0px)) 14px 8px",position:"sticky",top:0,background:"var(--c-bg, #040e20)",zIndex:5,borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
        ...
        <div style={{display:"flex",gap:6,position:"relative"}}>
          ...
          <button onClick={()=>setSortDir(d=>d==="asc"?"desc":"asc")}
            style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"6px 10px",color:"var(--c-accent-flug, #7dd3fc)",fontSize:12,cursor:"pointer"}}>
            {sortDir==="asc"?"↑":"↓"}
          </button>
          {showSortMenu && (
            <div style={{position:"absolute",top:"calc(100% + 4px)",left:0,right:0,background:"#14253a",border:"1px solid rgba(255,255,255,0.12)",borderRadius:10,padding:5,maxHeight:240,overflowY:"auto",zIndex:10,boxShadow:"0 8px 24px rgba(0,0,0,0.4)"}}>
              {SORT_OPTIONS.map(o=>(
                <div key={o.id} onClick={()=>{setSortId(o.id);setShowSortMenu(false);}}
                  style={{padding:"7px 10px",borderRadius:6,fontSize:12,cursor:"pointer",color:o.id===sortId?"var(--c-accent-flug, #7dd3fc)":"rgba(232,244,253,0.75)",background:o.id===sortId?"rgba(14,165,233,0.15)":"transparent"}}>
                  {o.label}
                </div>
              ))}
            </div>
      ...
      {sortId !== "date" ? (
        ...
        return (
          <div key={yr}>
            <div style={{padding:"8px 14px",fontSize:12,fontWeight:700,color:"var(--c-accent-flug, #7dd3fc)",background:"rgba(255,255,255,0.02)"}}>{yr} · {yFlights.length}</div>
            {yFlights.map(f => (
              <SidebarFlightRow key={f.id} f={f} selectedId={selectedId} longestId={longestId} onSelect={onSelect} />
            ))}
          </div>
  ...
  return (
    <div onClick={()=>onSelect(f)}
      ...
      <div style={{display:"flex",alignItems:"center",gap:6}}>
        ...
        <span style={{fontWeight:700,fontSize:13,color:"var(--c-text, #e8f4fd)"}}>{f.name}</span>
...
function FlugbuchApp() {
  ...
  // Palette + per-category alpha must match app.jsx's PALETTE_COLORS/
  // COLOR_CATEGORIES exactly (duplicated here since each page is a
  // standalone script with no shared module system) — this only reads the
  // colour-map the person configured in Einstellungen → 🎨 Farben and turns
  // it into CSS custom properties; nothing changes visually if they never
  // open that screen, since the fallback in every var(--x, fallback) below
  // is the original hardcoded colour.
  useEffect(() => {
    (async () => {
      try {
        ...
        const PALETTE = { white:"var(--c-text, #e8f4fd)", navy:"var(--c-bg, #040e20)", red:"#f87171", yellow:"#fde047",
          cyan:"var(--c-accent-flug, #7dd3fc)", bordeaux:"#e0304a", orange:"#f5a623", green:"#4ade80", violet:"#c4b5fd" };
        ...
        const CATS = [
          ...
          ["error","--c-error",1], ["rating","--c-rating",1], ["accentFlug","--c-accent-flug",1],
        ];
  ...
  const exportBackup = useCallback(async () => {
    ...
    let serviceData = {}, reisenData = {}, notesData = "", colorMapData = null;
    try {
      ...
      for (const k of (keys?.keys || [])) {
        ...
        } else if (k === "settings:colorMap") {
          const r = await window.storage.get(k);
          if (r) { try { colorMapData = JSON.parse(r.value); } catch {} }
        }
      }
  ...
  const importBackup = useCallback(async (file) => {
    try {
      ...
      if (data.colorMap && typeof data.colorMap === "object") {
        await window.storage.set("settings:colorMap", JSON.stringify(data.colorMap));
        restoredExtras++;
      }
  ...
  if (view==="detail" && selected && isWide) {
    return (
      <div style={{display:"flex",minHeight:"100vh",background:"var(--c-bg, #040e20)"}}>
    ...
    return (
      <div style={{maxWidth:480,margin:"0 auto",padding:"0 0 32px"}}>
        <div style={{display:"flex",alignItems:"center",gap:12,padding:"calc(16px + env(safe-area-inset-top, 0px)) 16px 12px"}}>
          <button onClick={()=>setView("detail")} style={{background:"none",border:"none",color:"var(--c-accent-flug, #7dd3fc)",fontSize:22,cursor:"pointer"}}>←</button>
        ...
        <div style={{padding:"0 16px"}}>
          {[["Name / Titel",editData.name||"","name"],["Startplatz",editData.site||"","site"],
            ...
            <div key={k} style={{marginBottom:12}}>
              ...
              <input value={v} onChange={e=>{
                ...
                style={{width:"100%",background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:10,padding:"10px 13px",color:"var(--c-text, #e8f4fd)",fontSize:14,boxSizing:"border-box"}} />
            </div>
          ))}
          ...
          <div style={{marginBottom:16}}>
            ...
            <textarea value={editData.notes||""} onChange={e=>setEditData(d=>({...d,notes:e.target.value}))} rows={2}
              style={{width:"100%",background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:10,padding:"10px 13px",color:"var(--c-text, #e8f4fd)",fontSize:13,resize:"vertical",boxSizing:"border-box"}} />
          </div>
  ...
  // ── LIST VIEW ─────────────────────────────────────────────────────────────
  return (
    <div style={{maxWidth:isWide?900:480,margin:"0 auto",minHeight:"100vh",background:"var(--c-bg, #040e20)",color:"var(--c-text, #e8f4fd)",fontFamily:"-apple-system,BlinkMacSystemFont,sans-serif"}}>
      ...
      {/* Header */}
      <div style={{position:"sticky",top:0,zIndex:10,background:"var(--c-bg, #040e20)"}}>
      ...
      {showImportMenu && (
        <div style={{margin:"8px 16px 0",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:10,padding:10,display:"flex",gap:8}}>
          <div onDragOver={e=>{e.preventDefault();setPdfDragOver(true)}} onDragLeave={()=>setPdfDragOver(false)}
            ...
            style={{flex:1,border:`2px dashed ${pdfDragOver?"var(--c-accent-flug, #7dd3fc)":"rgba(56,189,248,0.25)"}`,borderRadius:10,padding:"10px 8px",textAlign:"center",background:pdfDragOver?"rgba(56,189,248,0.08)":"transparent",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:3}}>
            <div style={{fontSize:15}}>📋</div>
            <div style={{color:pdfDragOver?"var(--c-accent-flug, #7dd3fc)":"rgba(125,211,252,0.5)",fontSize:10}}>CSV</div>
          </div>
      ...
      {selectMode && (
        <div style={{padding:"8px 16px 0",display:"flex",gap:8}}>
          ...
          <button onClick={()=>{
            ...
            style={{flex:"1 1 0",minWidth:0,boxSizing:"border-box",background:"rgba(14,165,233,0.15)",border:"1px solid rgba(14,165,233,0.3)",borderRadius:10,padding:"9px 4px",color:"var(--c-accent-flug, #7dd3fc)",fontSize:13,fontWeight:700,cursor:"pointer",textAlign:"center"}}>
            ✏️ {selectedIds.size}
          </button>
          <button onClick={()=>{
              if (!selectedIds.size) { setCopyMsg("Keine Flüge ausgewählt."); return; }
          ...
          <select
            ...
            <option value="" style={{background:"var(--c-bg, #040e20)"}}>🧭 {selectedIds.size}</option>
            {reisenNames.map(n => <option key={n} value={n} style={{background:"var(--c-bg, #040e20)"}}>{n}</option>)}
          </select>
        </div>
      )}
      {confirmBulkDelete && (
          ...
          <div onClick={e=>e.stopPropagation()}
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              ...
              <button onClick={()=>setConfirmBulkDelete(false)}
                style={{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:10,padding:"10px",color:"var(--c-text, #e8f4fd)",fontSize:14,cursor:"pointer"}}>Abbrechen</button>
            </div>
          </div>
        </div>
      )}
      ...
      {importProgress && (
        <div style={{position:"fixed",inset:0,background:"rgba(10,22,40,0.92)",zIndex:300,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:14}}>
          ...
          <div style={{fontSize:15,fontWeight:700,color:"var(--c-text, #e8f4fd)"}}>Speichere Flüge…</div>
          ...
          <div style={{width:200,height:6,background:"rgba(255,255,255,0.1)",borderRadius:10,overflow:"hidden"}}>
            <div style={{width:`${importProgress.total?Math.round(importProgress.done/importProgress.total*100):0}%`,height:"100%",background:"var(--c-accent-flug, #7dd3fc)",transition:"width 0.2s"}} />
          </div>
      ...
      <div style={{padding:"12px 16px 6px",position:"relative"}}>
        ...
        {showFilterHelp && (
          <div style={{marginTop:8,background:"rgba(125,211,252,0.07)",border:"1px solid rgba(125,211,252,0.2)",borderRadius:10,padding:"10px 12px",fontSize:11,lineHeight:1.6,color:"rgba(232,244,253,0.7)"}}>
            <div style={{fontWeight:700,color:"var(--c-accent-flug, #7dd3fc)",marginBottom:4}}>Filter-Syntax</div>
        ...
        {showSortMenu && (
          <div style={{marginTop:6,background:"#14253a",border:"1px solid rgba(255,255,255,0.12)",borderRadius:12,padding:6,maxHeight:280,overflowY:"auto",boxShadow:"0 8px 24px rgba(0,0,0,0.4)"}}>
            {SORT_OPTIONS.map(o=>(
              <div key={o.id} onClick={()=>{setSortId(o.id);setShowSortMenu(false);}}
                style={{padding:"9px 12px",borderRadius:8,fontSize:13,cursor:"pointer",color:o.id===sortId?"var(--c-accent-flug, #7dd3fc)":"rgba(232,244,253,0.75)",background:o.id===sortId?"rgba(14,165,233,0.15)":"transparent"}}>
                {o.label}
              </div>
            ))}
          </div>
        ...
        {showRowImport && (
          <div style={{marginTop:6,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:10,padding:10}}>
            <textarea value={rowImportText} onChange={e=>setRowImportText(e.target.value)}
              ...
              style={{width:"100%",minHeight:90,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:8,color:"var(--c-text, #e8f4fd)",fontSize:11,fontFamily:"monospace",boxSizing:"border-box",resize:"vertical"}} />
            {rowImportText.trim() && (()=>{
              ...
              <div onClick={()=>{
                ...
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  {selectMode && (() => {
                    ...
                    return (
                      <div style={{flexShrink:0,width:18,height:18,borderRadius:5,border:`2px solid ${allSelected?"var(--c-accent-flug, #7dd3fc)":"rgba(232,244,253,0.3)"}`,background:allSelected?"var(--c-accent-flug, #7dd3fc)":"transparent",display:"flex",alignItems:"center",justifyContent:"center"}}>
                        {allSelected && <span style={{color:"#0a1628",fontSize:11,fontWeight:900}}>✓</span>}
                      </div>
                    );
                  })()}
                  <span style={{fontWeight:700,color:"var(--c-accent-flug, #7dd3fc)",fontSize:14}}>{yr} · {yFlights.length} Flüge{yrBiplace>0&&<span style={{color:"#fcd34d",fontSize:11,fontWeight:600}}> · {yrBiplace} Biplace</span>}</span>
                </div>
