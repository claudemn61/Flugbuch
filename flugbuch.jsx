...
function FlightMap({ flight }) {
  ...
  const draw = (canvas) => {
    ...
    (async () => {
      ...
      const xMin = Math.floor(xMinF) - 1, xMax = Math.floor(xMaxF) + 1;
      const yMin = Math.floor(yMinF) - 1, yMax = Math.floor(yMaxF) + 1;
      ...
      // Position tiles using pure linear tile-space math (tile index minus
      // the fractional reference point, scaled to one consistent on-screen
      // tile size) instead of converting each tile's corners back to lon/lat
      // and re-projecting through the non-linear Mercator formula per tile.
      // Round-tripping every tile independently through
      // atan(sinh(...)) accumulates different rounding error per tile,
      // which showed up as visible gaps between otherwise-adjacent tiles
      // (the checkerboard pattern) even when every tile image loaded fine.
      const totalTileSpanX = xMaxF - xMinF;
      const totalTileSpanY = yMaxF - yMinF;
      const mapPixelW = W - 24, mapPixelH = H - 24;
      const tileScreenSize = Math.min(mapPixelW / totalTileSpanX, mapPixelH / totalTileSpanY);
      const drawnW = totalTileSpanX * tileScreenSize, drawnH = totalTileSpanY * tileScreenSize;
      const baseX = (W - drawnW) / 2 - (xMinF - xMin) * tileScreenSize;
      const baseY = (H - drawnH) / 2 - (yMinF - yMin) * tileScreenSize;
      ...
      tiles.forEach(({ xi, yi, img }) => {
        if (!img) return;
        anyLoaded = true;
        const px = baseX + (xi - xMin) * tileScreenSize;
        const py = baseY + (yi - yMin) * tileScreenSize;
        // Slight overdraw (+1px) on each tile so adjacent tiles overlap
        // by a hair rather than leaving a sub-pixel seam from canvas
        // anti-aliasing/rounding when drawImage lands on a fractional pixel.
        ctx.drawImage(img, Math.floor(px), Math.floor(py), Math.ceil(tileScreenSize)+1, Math.ceil(tileScreenSize)+1);
      });
      if (!anyLoaded) {
        ctx.fillStyle = "#0d1b2a"; ctx.fillRect(0,0,W,H);
      } else {
        ...
        >
          ...
          <button onClick={()=>{
              ...
              if (gpx) {
                ...
                a.href = url;
                a.download = `${flight?.name || "flug"}.gpx`;
            ...
            style={{position:"absolute",bottom:"calc(15vh + 14px)",left:14,background:"rgba(20,30,45,0.92)",border:"1px solid rgba(255,255,255,0.3)",borderRadius:20,padding:"8px 14px",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:5,boxShadow:"0 2px 8px rgba(0,0,0,0.4)"}}>
            ⬇ GPX
          </button>
