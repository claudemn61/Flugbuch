...
function SearchBar({ filterText, setFilterText }) {
  // Explicit open/close (not derived from the text field), since deriving
  // it from "is there text" caused the panel to pop open/closed on every
  // keystroke — every open/close is now a deliberate tap: the ☰ button
  // opens it, the ✓ button closes it again.
  ...
  return (
    <div style={{position:"relative"}}>
      <div style={{display:"flex",gap:6,alignItems:"center"}}>
        ...
        <button onClick={()=>setAdvOpen(o=>!o)} title="Erweiterte Suche"
          style={{background:advOpen?"rgba(14,165,233,0.18)":"rgba(255,255,255,0.06)",border:`1px solid ${advOpen?"rgba(14,165,233,0.4)":"rgba(255,255,255,0.1)"}`,borderRadius:8,width:34,height:34,color:advOpen?"#7dd3fc":"rgba(232,244,253,0.6)",fontSize:13,cursor:"pointer",flexShrink:0}}>
          ☰
        </button>
      </div>
...
function CoordEdit({lat, lon, alt, color, onSave}) {
  ...
  if (editing) {
    return (
      <div>
        <input value={combined} onChange={e=>setCombined(e.target.value)} placeholder="Lat, Lon (z.B. 47.21990, 8.45354)" autoFocus style={iStyle} />
