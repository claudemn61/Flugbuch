...
function SearchBar({ filterText, setFilterText }) {
  // No separate toggle: the advanced row-builder simply appears as soon as
  // there's something typed in the field, and collapses back to a single
  // line once the field is cleared. The plain text input keeps working for
  // freeform queries the whole time, independent of whatever rows show.
  const advOpen = filterText.trim().length > 0;
      ...
      {selectMode && (
        <div style={{padding:"8px 16px 0",display:"flex",gap:8}}>
          ...
          <button onClick={()=>{
            ...
            style={{flex:1,background:"rgba(14,165,233,0.15)",border:"1px solid rgba(14,165,233,0.3)",borderRadius:10,padding:"9px 6px",color:"#7dd3fc",fontSize:13,fontWeight:700,cursor:"pointer",textAlign:"center"}}>
            ✏️ {selectedIds.size}
          </button>
          <button onClick={()=>{
              if (!selectedIds.size) { setCopyMsg("Keine Flüge ausgewählt."); return; }
