...
function createFlightFromPDF(nr, p) {
  ...
  if (durStr) {
    ...
    else { const dm2=durStr.match(/(\d+):(\d{2})/); if(dm2) durationSec=+dm2[1]*3600 + +dm2[2]*60; }
  }
...
function FlugbuchApp() {
  ...
  const applyParsedData = useCallback(async (DATA) => {
    ...
    const updatedFlights = flights.map(f => {
      ...
      let durationSec;
      if (dm) durationSec = +dm[1]*3600 + +dm[2]*60 + +dm[3];
      else { const dm2=(p.dur||"").match(/(\d+):(\d{2})/); durationSec = dm2 ? +dm2[1]*3600 + +dm2[2]*60 : f.durationSec; }
      return {
        ...f,
        site: p.st || f.site,
  ...
  const importPDFFile = useCallback(async (file) => {
    ...
    if (file.name.toLowerCase().endsWith(".csv")) {
      ...
      try {
        ...
        // TEMP DEBUG: show the raw "dur" value parsed for the first 5 flight
        // numbers, so we can see exactly what parseCSV extracted from column
        // 35 before it gets turned into durationSec.
        const sampleKeys = Object.keys(parsed).slice(0, 5);
        const sample = sampleKeys.map(k => `${k}: dur="${parsed[k].dur}"`).join(" | ");
        console.log("[DEBUG CSV] Sample dur values:", sample);
        await applyParsedData(parsed);
        setPdfResult(r => ({ ...r, debugDur: sample }));
  ...
  return (
    <div style={{maxWidth:isWide?900:480,margin:"0 auto",minHeight:"100vh",background:"#040e20",color:"#e8f4fd",fontFamily:"-apple-system,BlinkMacSystemFont,sans-serif"}}>
      ...
      {pdfResult?.debugDur && (
        <div style={{margin:"6px 16px 0",fontSize:10,color:"#fcd34d",fontFamily:"monospace",wordBreak:"break-all"}}>
          DEBUG: {pdfResult.debugDur}
        </div>
      )}
