...
function FlightMap({ flight }) {
  ...
  const draw = (canvas) => {
    ...
    const drawTrackAndMarkers = () => {
      ...
      if (track.length) {
        ...
        for(let i=1;i<track.length;i++){
          ...
          ctx.strokeStyle="rgba(255,255,255,0.55)"; ctx.lineWidth=4.5;
    ...
    (async () => {
      ...
      const xMin = Math.floor(Math.min(p1.x, p2.x)), xMax = Math.floor(Math.max(p1.x, p2.x));
      const yMin = Math.floor(Math.min(p1.y, p2.y)), yMax = Math.floor(Math.max(p1.y, p2.y));
  ...
  useEffect(() => {
    ...
    // and needs to re-run whenever the overlay actually mounts.
    const canvas = fullCanvasRef.current;
    if (canvas) {
      canvas.width = window.innerWidth * (window.devicePixelRatio||1);
      canvas.height = window.innerHeight * 0.7 * (window.devicePixelRatio||1);
    }
