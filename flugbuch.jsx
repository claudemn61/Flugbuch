...
function FlightProfile({ flight }) {
  ...
  // Displayed instead of the raw flown distance — see composeXcDistance's
  // own comment for what this approximates and why it isn't exact.
  const xcDist = useMemo(() => composeXcDistance(track), [track]);
    ...
    (async () => {
      try {
        ...
        if (Array.isArray(data.elevation)) {
          setGroundProfile(samplePts.map((s,i) => ({ distKm: s.distKm, elev: data.elevation[i] })));
        } else {
          setGroundError(true);
        }
  ...
  useEffect(() => {
    ...
    ctx.textAlign = "right"; ctx.fillText(xcDist.toFixed(1)+" km", padL+plotW, padT+plotH+15*dpr);
  ...
  }, [track, distances, totalDist, xcDist, groundProfile]);
  ...
  return (
    <div style={{marginBottom:14}}>
      ...
      <div style={{fontSize:9,color:"rgba(232,244,253,0.3)",marginTop:4}}>Streckenangabe: angenäherte zusammengesetzte Strecke (2 Wendepunkte), nicht identisch mit XContest-Wert.</div>
    </div>
  );
}
...
// Rough approximation of XContest's "zusammengesetzte Strecke" (free
// distance via up to 2 turnpoints): finds the 2 in-order track points that,
// together with launch and landing, maximise the sum of the 3 straight-line
// legs (launch→TP1→TP2→landing). This is NOT the real XContest algorithm —
// that's a genuine O(n³) optimisation problem with specialised geometry
// (see igc-xc-score, the reference implementation XContest itself uses) —
// but a brute-force O(n²) search over a downsampled track gets reasonably
// close and runs in milliseconds on a phone, which the real algorithm's
// data structures aren't built for doing client-side without a dedicated
// library. Always ≤ the raw flown track length (triangle inequality).
function composeXcDistance(track) {
  if (!track || track.length < 3) return 0;
  const maxPts = 250;
  const step = Math.max(1, Math.ceil(track.length/maxPts));
  const pts = [];
  ...
  for (let i=1;i<pts.length-1;i++) {
    const dStart = haversineDistKm(start, pts[i]);
    for (let j=i+1;j<pts.length-1;j++) {
      const total = dStart + haversineDistKm(pts[i],pts[j]) + haversineDistKm(pts[j],end);
