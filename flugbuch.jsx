...
const SEARCH_FIELDS = [
  ...
  { id: "passagier", label: "Passagier",      type: "text" },
  { id: "datum",     label: "Datum",          type: "text" },
...
function buildAdvancedQuery(rows, combine) {
  const parts = rows
    ...
    .map(r => {
      ...
      const op = fieldDef?.type === "number" ? (r.op || "=") : ":";
                ...
                <div key={idx} style={{display:"flex",gap:6,alignItems:"center"}}>
                  ...
                  <select value={row.field}
                    onChange={e=>{
                      ...
                      updateRow(idx, { field: e.target.value, op: nf?.type==="number" ? "=" : undefined });
                    }}
                  ...
                  {fieldDef?.type === "number" && (
                    <select value={row.op||"="} onChange={e=>updateRow(idx,{op:e.target.value})}
                      style={{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:8,padding:"5px 2px",color:"#e8f4fd",fontSize:12,width:50,flexShrink:0}}>
                      {ADV_NUM_OPS.map(o=><option key={o} value={o} style={{background:"#0a1628"}}>{o}</option>)}
                    </select>
                  )}
                  <input value={row.value} onChange={e=>updateRow(idx,{value:e.target.value})} placeholder="Wert…"
    ...
    return (
      <div onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}
        ...
        <div style={{padding:"0 16px"}}>
          ...
          {/* Bemerkung — moved up here, directly under the rating and above
              the map, per request; still the same InlineField/saveField. */}
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:14}}>
            {[
              ...
              <div key={l} style={{background:"rgba(255,255,255,0.05)",borderRadius:10,padding:"7px 6px",textAlign:"center",border:"1px solid rgba(255,255,255,0.06)"}}>
                <div style={{fontSize:12,marginBottom:1}}>{ic}</div>
                <div style={{fontSize:14,fontWeight:800,color:"#7dd3fc"}}>{v}</div>
                <div style={{fontSize:8,color:"rgba(232,244,253,0.4)",marginTop:1,textTransform:"uppercase",letterSpacing:0.4}}>{l}</div>
              </div>
            ))}
          </div>
            ...
            <div style={{background:"rgba(34,197,94,0.07)",borderRadius:12,padding:"10px",border:"1px solid rgba(34,197,94,0.18)"}}>
              ...
              <CoordEdit
                ...
                onSave={(lat,lon,alt)=>{
                  ...
                  saveField({startPt:sp, startAlt:alt||0});
                }} />
            </div>
            <div style={{background:"rgba(239,68,68,0.07)",borderRadius:12,padding:"10px",border:"1px solid rgba(239,68,68,0.18)"}}>
              ...
              <CoordEdit
                ...
                onSave={(lat,lon,alt)=>{
                  ...
                  saveField({endPt:ep, endAlt:alt||0});
                }} />
            </div>
          </div>
...
function FlugbuchApp() {
  ...
  return (
    <div style={{maxWidth:isWide?900:480,margin:"0 auto",minHeight:"100vh",background:"#040e20",color:"#e8f4fd",fontFamily:"-apple-system,BlinkMacSystemFont,sans-serif"}}>
      ...
      {/* Row 2: Import / Backup / Auswahl / Saison — 4 equal buttons */}
      <div style={{padding:"10px 16px 0",display:"flex",gap:8}}>
        <button onClick={()=>{ setShowImportMenu(m=>!m); setShowBackupMenu(false); }}
          style={{flex:"1 1 0",minWidth:0,boxSizing:"border-box",background:showImportMenu?"rgba(56,189,248,0.15)":"rgba(255,255,255,0.05)",border:`1px solid ${showImportMenu?"rgba(56,189,248,0.35)":"rgba(255,255,255,0.1)"}`,borderRadius:10,padding:"9px 4px",color:"#fff",fontSize:12,fontWeight:600,cursor:"pointer",textAlign:"center"}}>
          📥 Import {showImportMenu?"▾":"▸"}
        </button>
        <button onClick={()=>{ setShowBackupMenu(m=>!m); setShowImportMenu(false); }}
          style={{flex:"1 1 0",minWidth:0,boxSizing:"border-box",background:showBackupMenu?"rgba(56,189,248,0.15)":"rgba(255,255,255,0.05)",border:`1px solid ${showBackupMenu?"rgba(56,189,248,0.35)":"rgba(255,255,255,0.1)"}`,borderRadius:10,padding:"9px 4px",color:"#fff",fontSize:12,fontWeight:600,cursor:"pointer",textAlign:"center"}}>
          💾 Backup {showBackupMenu?"▾":"▸"}
        </button>
        <button onClick={()=>{ setSelectMode(m=>!m); setSelectedIds(new Set()); setCopyMsg(""); }}
          style={{flex:"1 1 0",minWidth:0,boxSizing:"border-box",background:selectMode?"rgba(14,165,233,0.18)":"rgba(255,255,255,0.05)",border:`1px solid ${selectMode?"rgba(14,165,233,0.4)":"rgba(255,255,255,0.1)"}`,borderRadius:10,padding:"9px 4px",color:"#fff",fontSize:12,fontWeight:600,cursor:"pointer",textAlign:"center"}}>
          {selectMode?"✕ Auswahl":"☑ Auswahl"}
        </button>
        <button onClick={()=>setView("season")}
          style={{flex:"1 1 0",minWidth:0,boxSizing:"border-box",background:"rgba(245,158,11,0.15)",border:"1px solid rgba(245,158,11,0.25)",borderRadius:10,padding:"9px 4px",color:"#fff",fontSize:12,fontWeight:600,cursor:"pointer",textAlign:"center"}}>
          📊 Saison
        </button>
      </div>
      ...
      {/* Row 3: Suchen / Sortierung / Richtung / Jahr auf-zu — Suchen+Sortierung gleich breit, Richtung+Jahr je halb so breit */}
      <div style={{padding:"12px 16px 6px",position:"relative"}}>
        <div style={{display:"flex",gap:8,alignItems:"flex-start"}}>
          ...
          <button onClick={()=>setShowSortMenu(s=>!s)}
            style={{flex:"2 1 0",minWidth:0,boxSizing:"border-box",display:"flex",justifyContent:"space-between",alignItems:"center",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,padding:"8px 8px",color:"#fff",fontSize:12,cursor:"pointer"}}>
          ...
          <button onClick={()=>setSortDir(d=>d==="asc"?"desc":"asc")}
            ...
            style={{flex:"1 1 0",minWidth:0,boxSizing:"border-box",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,padding:"8px 4px",color:"#fff",fontSize:14,cursor:"pointer",textAlign:"center"}}>
            {sortDir==="asc"?"↑":"↓"}
          </button>
          <button
            onClick={()=>setCollapsedYears(s=>s.size===0?new Set(years):new Set())}
            title={collapsedYears.size===0?"Alle reduzieren":"Alle erweitern"}
            style={{flex:"1 1 0",minWidth:0,boxSizing:"border-box",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,padding:"8px 4px",color:"#fff",fontSize:11,fontWeight:700,cursor:"pointer",letterSpacing:1,textAlign:"center"}}>
