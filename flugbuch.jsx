...
function flightToCsvRow(f) {
  ...
  const val = {
    ...
    bemerkung: f.notes || "",
    bewertung: f.rating ? String(f.rating) : "",
  };
  ...
  const row = [
    ...
    val.bewertung,              // 54 Bewertung (Sterne, 1-5) — neu angehängt, verschiebt keine bestehende Spalte
  ];
  return row.join("\t");
}
...
const CSV_HEADER = [
  ...
  "Gerät", "Passagier", "Datum2", "Bemerkung", "Bewertung",
].join("\t");
...
function parseCompactNumbersRow(cols) {
  ...
  return {
    ...
    rt: get(25), // Bewertung (Sterne) — leer bei älteren Exports ohne diese Spalte
    _nr: get(0),
  ...
  const applyParsedData = useCallback(async (DATA) => {
    ...
    const updatedFlights = flights.map(f => {
      ...
      return {
        ...
        notes: p.be || f.notes,
        rating: f.rating || (parseInt(p.rt, 10) || 0),
