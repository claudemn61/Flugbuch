...
function analyzeIGC(track) {
  ...
  return { maxAlt, minAlt, startAlt, endAlt, startPt, endPt, durationSec,
    totalDist: +(totalDist/1000).toFixed(2), thermalCount: thermals.length, maxClimb, totalGain: Math.round(totalGain) };
}
    ...
    for (let i=0; i<igcFiles.length; i++) {
      ...
      if (existing) {
        ...
        const newF = { id:`igc_${baseName}_${Date.now()}`, name:baseName, pdfOnly:false,
          ...
          customFields:{passagier:passagier||"",landung:"",hGew: igcData.totalGain ? String(igcData.totalGain) : ""},
