...
function evalToken(f, tok){
  ...
  if(m){
    ...
    if((field==="passagier"||field==="pax") && raw==="*"){
      return !!(f.customFields?.passagier||"").trim();
    }
    ...
    const numericFields=["dauer","duration","distanz","dist","km","höhe","hoehe","maxhöhe","maxhoehe","alt",
      ...
      "speed","kmh","rating","bewertung","jahr","year"];
...
const SEARCH_FIELDS = [
  ...
  { id: "startzeit", label: "Startzeit",      type: "text" },
  { id: "landezeit", label: "Landezeit",      type: "text" },
              ...
              return (
                <div key={idx} style={{display:"flex",gap:6,alignItems:"center"}}>
                  ...
                  <input value={row.value==="*"?"":row.value} onChange={e=>updateRow(idx,{value:e.target.value})}
                    placeholder={fieldDef?.anyOption ? "Name, oder \"beliebig\" →" : "Wert…"}
                    disabled={row.value==="*"}
        ...
        <div style={{padding:"0 16px"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:14}}>
            {(() => {
              ...
              const openFlugdaten = () => {
                document.getElementById("flugdaten-section")?.scrollIntoView({behavior:"smooth", block:"start"});
              };
              return shown.map(([ic,v,l])=>(
                <div key={l} onClick={openFlugdaten}
