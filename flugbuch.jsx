  ...
  const processIGCFiles = useCallback(async (igcFiles) => {
    ...
    for (let i=0; i<igcFiles.length; i++) {
      ...
      if (existing) {
        ...
        console.log("[DEBUG IGC] Saving existing flight", updated.id, "track length:", updated.track?.length);
        await saveFlight(updated);
        const verify = await window.storage.get(`flight:${updated.id}`);
        console.log("[DEBUG IGC] Verify after save, track length in storage:", verify ? JSON.parse(verify.value).track?.length : "NOT FOUND");
    ...
    <div style={{maxWidth:isWide?900:480,margin:"0 auto",minHeight:"100vh",background:"#0a1628",color:"#e8f4fd",fontFamily:"-apple-system,BlinkMacSystemFont,sans-serif"}}>
      ...
      <div style={{background:"rgba(255,255,255,0.03)",borderBottom:"1px solid rgba(255,255,255,0.06)",padding:"14px 16px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:10,backdropFilter:"blur(10px)"}}>
        <span onClick={()=>{window.location.href="index.html";}}
          style={{fontWeight:900,fontSize:18,letterSpacing:-0.5,cursor:"pointer",display:"flex",alignItems:"center",gap:4}}>
          <span style={{fontSize:15,opacity:0.5}}>‹</span>✈️ Flugbuch
        </span>
        <div style={{display:"flex",gap:8}}>
      ...
      <div style={{padding:"6px 16px 0",fontSize:10,color:"#fcd34d",fontFamily:"monospace"}}>
        DEBUG Tracks: {flights.filter(f=>f.track&&f.track.length>1).map(f=>f.name+"("+f.track.length+")").join(", ")||"keine"}
      </div>
