...
function analyzeIGC(track, tzOffsetHours, dateStr) {
  ...
  const durationStr = `${durH}h ${String(durM).padStart(2,"0")}m ${String(durS).padStart(2,"0")}s`;
...
function FlightMap({ flight }) {
  ...
  const openInGpsVisualizer = (e) => {
    ...
    // and the track just renders gray. Rebuilding a real UTC timestamp per
    // point from the flight's date + each point's seconds-since-midnight.
    const dm = String(flight?.rawDate || flight?.date || "").match(/(\d{1,2})\.(\d{1,2})\.(\d{4})/);
    const baseUtcMs = dm ? Date.UTC(+dm[3], +dm[2]-1, +dm[1]) : null;
    const rows = ["type,latitude,longitude,altitude,time"];
    for (let i = 0; i < track.length; i += step) {
      const p = track[i];
      const iso = baseUtcMs != null ? new Date(baseUtcMs + p.timeSec*1000).toISOString() : "";
    ...
    const saveComputedField = async (currentFl, patch) => {
      ...
      if (upd.startTime && upd.endTime) {
        ...
        if (diffSec > 0) {
          ...
          upd.durationStr = `${h}h ${String(m).padStart(2,"0")}m ${String(s).padStart(2,"0")}s`;
        }
      }
