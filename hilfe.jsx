  ...
  },
  {
    ...
    body: () => (<>
      ...
      <ul>
        ...
        <li>📈 Graph (cyan) — zeigt die gerade in der Flugliste aktive Filterung als Diagramm, zwei umschaltbare Modi: <b>Gruppiert</b> (Balken zur Gr. 1° der Flugliste, X-/Y-Achse danach im Badge frei wählbar — X: dieselben Felder wie Gr. 1°/2° im Flugbuch, Y: Anzahl Flüge oder Gesamt/Ø/Max. jedes numerischen Flugdatenfelds) und <b>Frei</b> (ein Punkt pro Flug, X und Y teilen sich dieselbe Feldliste — Datum/Jahr/Monat/Std. oder jedes numerische Feld —, als Linie oder nur Punkte) — startet automatisch im passenden Modus, je nachdem ob in der Flugliste eine Gr. 1° aktiv ist. Beide Achsen (X und Y, in beiden Modi) per ⇅ umkehrbar. Die Achsen-Beschriftungen zeigen dabei immer nur den gerade sichtbaren Ausschnitt in regelmässigen, "runden" Abständen (ganzzahlig bzw. glatte Schritte; bei Datum je nach Spanne auf Jahres-, Monats- oder Tages-Raster ausgerichtet). Ist in der Flugliste Gr. 2° gesetzt, kommt in beiden Modi ein "Aufschlüsseln nach…"-Dropdown dazu, das auf einen einzelnen Gr.-2°-Wert einschränkt. Werte, die bei der gewählten Y-Achse leer/0 sind, lassen sich ausblenden. Echter Bereichs-Zoom per Pinch (zwei Finger verengen den sichtbaren Werte-/Zeitbereich, Balken/Punkte werden dadurch tatsächlich grösser statt nur optisch vergrössert) plus Verschieben (ein Finger), Doppeltipp oder ⤾-Symbol setzt auf die volle Spanne zurück. ↻ bzw. Tippen auf die Bezug-Zeile führt zur Flugliste (inkl. aktuellem Filter) — von dort führt der 📊-Button oben links direkt wieder zurück zum offenen Graph-Badge.</li>
      </ul>
    </>),
  },
  {
...
function KurzContent() {
  return (
    <div className="hilfe-body">
      ...
      <h3>Statistik</h3>
      <p>7 Badges (Passagiere/Hike blenden sich einzeln aus, wenn nicht zutreffend): Schirm · Startplätze · Landeplätze · Passagiere · Hike (gruppiert nach Ort) · Saison (Jahresübersicht + Rekorde) · Graph (Gruppiert: Balken zur Flugliste-Gruppierung; Frei: Linie/Punkte je Flug — beide mit frei wähl- und umkehrbarer X-/Y-Achse (gleiche Feldliste), runden Achsen-Ticks, Gr.-2°-Aufschlüsselung, echtem Bereichs-Zoom per Pinch)</p>
      <h3>Reisen</h3>
