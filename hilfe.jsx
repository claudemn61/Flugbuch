...
const SECTIONS = [
  {
    ...
    body: () => (<>
      ...
      <table><tbody>
        ...
        <tr><td>🎒</td><td><b>Ausrüstung, Brevet</b></td><td>Drei Tabs: <b>⚖️ Gewichte</b> (Material in 8 Kategorien, Setups mit Gesamtgewicht/Limite), <b>🛠️ Wartung</b> (Reserve/Schirm/Sitz, Check-Intervall und fälliges Check-Datum) und <b>🎓 Brevet</b> (Ausweise mit Foto, Perspektiv-Korrektur beim Zuschneiden)</td></tr>
      </tbody></table>
  ...
  },
  {
    ...
    body: () => (<>
      ...
      <p>Ist bei einem Flug eine der drei automatisch bestimmbaren Routenarten hinterlegt, erscheint direkt vor der Distanz-Anzeige (egal ob diese als Haupt- oder Nebenwert steht, je nach aktueller Sortierung) ein kleines Symbol: ▲ für ein Dreieck (grün bei FAI-Dreieck, sonst in der Farbe der Distanz-Anzeige selbst), ↗ für eine Freie Strecke.</p>
      <h3>3.2 Suche</h3>
      ...
      <p>Bei aktiver Suche oder aktiver Darstellung (siehe 3.1) steht neben der Trefferzahl ein aufklappbares Statistik-Feld (▸/▾) zur aktuellen Auswahl. Standardmässig aktiv: Zeitraum, Gesamtzeit, Gesamtdistanz, Ø Dauer, Ø Distanz, längster Flug, weitester Flug, grösste Höhe, Anzahl Startplätze, Anzahl Schirme. Zusätzlich verfügbar, aber erst nach Auswahl sichtbar: Anzahl Landeplätze, Flugtage, Anzahl Biplace-Flüge, Ø Höhengewinn, Max. Steigen, Max. Sinken. Bei aufgeklapptem Feld erscheint zusätzlich ein ⚙️-Zahnrad, über das sich auswählen lässt, welche dieser Werte angezeigt werden und in welcher Reihenfolge — an-/abwählen, per ↑/↓ neu anordnen, wie bei den Spalten fürs Kopieren.</p>
      ...
      <p><b>Distanz und Routenart sind Vorschläge, keine automatischen Endwerte</b> — beide bleiben frei überschreibbar und werden nur ins jeweils leere Feld eingetragen. Distanz: die App berechnet sowohl die freie Strecke (Kette über bis zu 3 Wendepunkte) als auch das bestbewertete geschlossene Dreieck im Track und schlägt vor, was nach XContest-Bonuspunkten (1.0×/1.2×/1.4×/1.6×) gewinnt — dieselbe Logik wie XContests eigene Bewertung. Routenart zeigt entsprechend "Freie Strecke", "Flaches Dreieck" oder "FAI-Dreieck". Da XContest-Werte (Streckenoptimierung über Wendepunkte) massgeblich bleiben und der eigene Suchalgorithmus nicht immer exakt trifft, lohnt sich ein Abgleich mit dem königsblauen "XContest"-Button neben dem IGC-Badge im Flugdetail. Ein erneuter Import befüllt nur leere Felder; Dauer und Höhendifferenz werden immer neu berechnet, da rein rechnerisch.</p>
      <p>Bei vorhandenem GPS-Track zeigt das Flugdetail unter Distanz zusätzlich drei Kennwerte: Max. Freie Strecke, Max. Flaches Dreieck und Max. FAI-Dreieck, je mit den dazugehörigen XContest-Punkten. Wird die Routenart danach manuell umgestellt (Freie Strecke/Flaches Dreieck/FAI-Dreieck), passen sich Distanz und die auf der Karte eingezeichnete Route automatisch auf den entsprechend berechneten Wert an — beide bleiben aber weiterhin frei überschreibbar, auch direkt durch Verschieben der Punkte auf der Karte (siehe 3.6).</p>
      <p>Max. Steigen/Sinken wird über die Druckhöhe (barometrisch, falls vom Logger aufgezeichnet, sonst GPS-Höhe) ermittelt: die Sekunden-Steigraten werden mit einem gleitenden Mittelwert über 30 Samples geglättet, der höchste/tiefste geglättete Wert im Flug gilt — dieses Verfahren wurde anhand eigener Flüge mit bekannten XContest-Werten empirisch bestimmt und trifft diese exakt.</p>
      ...
      <p>Karte (MapTiler, Geländestil mit deutscher Beschriftung) zeigt den Track in kräftigem Dunkelblau (nicht höhen-/steigenkodiert); Vollbild per Antippen mit echtem Pinch-/Doppeltipp-Zoom, Link zu GPS Visualizer. Die automatisch erkannte Route (freie Strecke oder Dreieck, siehe 3.4) erscheint als gestrichelte gelbe Linie über dem Track, mit nummerierten Punkten an den Wendepunkten — bei einem Dreieck zusätzlich eine gleich gestaltete Linie vom letzten zurück zum ersten Punkt (echte dritte Seite, zählt mit zur Distanz). Im Vollbild lassen sich die nummerierten Punkte per Antippen und Ziehen verschieben, per "+ Punkt"-Knopf neue hinzufügen und per ×-Symbol löschen — Distanz und Routenart im Flugdetail werden dabei sofort neu berechnet, falls die automatische Erkennung danebenliegt. Der Referenzpunkt (bei aktivem Profil-Zoom) ist ein stilisierter Gleitschirm, der sich in die tatsächliche Flugrichtung dreht. Cine-Wiedergabe (▶-Button, auch schon in der kleinen Vorschau, nicht nur im Vollbild) spielt den Flug mit wählbarer Geschwindigkeit (1×–100×) ab — der Gleitschirm-Marker folgt der Strecke und dreht sich live mit; im Vollbild steht zusätzlich die aktuelle Höhe (rot) direkt neben dem Marker. Bei aktivem Profil-Zoom springen Karte und Höhenprofil während der Wiedergabe automatisch mit, sobald der Marker den sichtbaren Bereich verlässt. ↺ springt zurück zum Start. Höhenprofil zeigt Höhe über Distanz höhenfarbig (rot=tief, blau=hoch) mit braunem Bodenprofil (echte Geländedaten, 80 Stützpunkte), proportional auf die eingetragene Distanz skaliert.</p>
      <ul>
  ...
  },
  {
    ...
    body: () => (<>
      <p>Sieben farbige Badges (ab 768px Breite nebeneinander statt untereinander):</p>
      <ul>
        ...
        <li>📈 Graph (cyan) — öffnet anders als die übrigen Badges formatfüllend über der ganzen Seite statt eingeklappt darunter, die Zeichenfläche misst ihre Breite laufend und nutzt beim Drehen ins Querformat automatisch den ganzen Bildschirm. Zeigt die gerade in der Flugliste aktive Filterung als Diagramm, zwei umschaltbare Modi: <b>Gruppiert</b> (Balken zur Gr. 1° der Flugliste, X-/Y-Achse danach im Badge frei wählbar — X: dieselben Felder wie Gr. 1°/2° im Flugbuch plus alle numerischen Flugdatenfelder inkl. Flugnummer, X-Achse wahlweise nach eigenem Wert (bei Namensfeldern wie Reise/Schirm alphabetisch, sonst z.B. chronologisch) oder nach dem angezeigten Y-Wert sortierbar — so lässt sich z.B. eine Rangfolge "Jahre mit den längsten Flügen" statt der chronologischen Reihenfolge anzeigen; Y: Anzahl Flüge oder Gesamt/Ø/Max. jedes numerischen Flugdatenfelds inkl. Flugnummer/Jahr/Monat/Std.) und <b>Frei</b> (ein Punkt pro Flug, X und Y teilen sich dieselbe Feldliste — Flugnummer/Datum/Jahr/Monat/Std. oder jedes numerische Feld) — startet automatisch im passenden Modus, je nachdem ob in der Flugliste eine Gr. 1° aktiv ist. Jede Achse zeigt nur noch das Auswahl-Dropdown; ein fett hervorgehobener "X"- bzw. "Y"-Button links daneben öffnet ein Popup mit den übrigen Optionen dieser Achse: Reihenfolge umkehren (⇅), Sortierung (bei Gruppiert: eigener Wert/A–Z oder nach Y-Wert — auch bei numerischen Feldern wie Jahr, z.B. für eine Rangfolge "Jahre mit den längsten Flügen"; bei Frei: "Wert" oder "Rang" — bei "Rang" wird die Achse statt am eigenen Feldwert nach dem Rang der jeweils anderen Achse angeordnet, z.B. X = Rang nach Distanz statt Flugnummer, ergibt eine sortierte Verteilungskurve statt eines Streudiagramms) und ⚙️ Auswahlliste bearbeiten (Häkchen + ↑/↓ für die Feldliste dieser Achse, bleibt über einen Neustart hinweg erhalten). Die Achsen-Beschriftungen zeigen dabei immer nur den gerade sichtbaren Ausschnitt in regelmässigen, "runden" Abständen (ganzzahlig bzw. glatte Schritte; bei Datum je nach Spanne auf Jahres-, Monats- oder Tages-Raster ausgerichtet). Ist in der Flugliste Gr. 2° gesetzt, kommt in beiden Modi neben der Trendlinie-Checkbox eine kleine Kachel mit dem Feldnamen (z.B. "Monat") dazu — ein Klick öffnet ein Popup mit der Werteliste, eingeschränkt auf einen einzelnen Gr.-2°-Wert. Werte, die bei der gewählten Y-Achse leer/0 sind, lassen sich ausblenden. Zusätzlich eine optionale gestrichelte Trendlinie (lineare Regression), nur eingeblendet wenn statistisch sinnvoll (mindestens 3 Punkte, Streuung in X — bei kategorischem X in Gruppiert daher nur bei numerischem X), per Checkbox ausblendbar. Echter Bereichs-Zoom per Pinch (zwei Finger verengen den sichtbaren Werte-/Zeitbereich, Balken/Punkte werden dadurch tatsächlich grösser statt nur optisch vergrössert) plus Verschieben mit einem Finger (X und Y: Inhalt folgt dem Finger), Doppeltipp oder ⤾-Symbol setzt auf die volle Spanne zurück. Im Modus <b>Frei</b> lässt sich, sobald gezoomt ist, ein einzelner Punkt antippen — zeigt seine Flugnummer über dem Punkt; ein weiterer Tipp auf die Nummer öffnet den Flug direkt im Flugdetail (der Zurück-Pfeil dort führt wieder zu diesem offenen Graph-Badge zurück). ↻ bzw. Tippen auf die Bezug-Zeile führt zur Flugliste (inkl. aktuellem Filter) — von dort führt der 📊-Button oben links direkt wieder zurück zum offenen Graph-Badge.</li>
      </ul>
    </>),
  },
  {
  ...
  {
    id: "ausruestung", title: "6. Ausrüstung, Brevet",
    body: () => (<>
      <p>Drei Tabs oben, im Design analog Wartungs eigenem Reserve/Schirm/Sitz-Umschalter: <b>⚖️ Ausrüstung, Gewichte</b>, <b>🛠️ Wartung</b> und <b>🎓 Brevet</b> — jeweils nur einer aktiv.</p>
      ...
      <p>Mehrere <b>Setups</b> (z.B. "Tandem", "Solo", "H&F leicht") sind frei anlegbar ("+ Setup") und umbenennbar (Titel antippen). Auf iPad/Mac stehen alle Setups nebeneinander als Spalten, auf dem iPhone per Tab-Umschalter (nur eins sichtbar). Jedes Setup wählt per Checkbox unabhängig aus, welche Positionen mitzählen. Angezeigt werden: Gesamtgewicht (mit "Rucksack"-Teilgewicht ohne Körpergewicht/Kleidung), Reserve und Fläch.-Belastung — beide aus der beim Schirm angehakten Gewichtslimite bzw. Proj. Fläche berechnet. Die Gewichtslimite eines Schirms ist ein Range (Min–Max); unter Reserve steht zusätzlich klein und grau, wie viel Prozent dieser Range das aktuelle Gesamtgewicht ausmacht (0% = Min, 100% = Max, auch negativ oder über 100% möglich). ✏️/✓ auf jeder Kategorie-Zeile schaltet global zwischen Übersicht (nur angehakte Positionen) und Bearbeiten (alle Positionen, Duplizieren, Löschen) um.</p>
      <h4>🛠️ Wartung</h4>
      ...
      <p>Die Titel der einzelnen Positionen sind direkt editierbar: auf einen bereits ausgewählten (hervorgehobenen) Titel nochmals tippen (bzw. im breiten Layout den Titel antippen) öffnet eine Namens-Liste — bei Schirm vorbefüllt mit allen im Flugbuch vorkommenden Schirm-Namen, bei Reserve/Sitz mit den bereits anderswo in dieser Kategorie vergebenen Namen. Alternativ lässt sich unten ein neuer, eigener Name eintippen; "↺ Zurücksetzen" leert den Titel wieder auf den generischen Platzhalter ("Reserve 1", "Schirm 1" usw.). Ab 768px Breite sind alle Positionen einer Kategorie gleichzeitig als feste Spalten sichtbar, statt einzeln per Tab.</p>
      <h4>🎓 Brevet</h4>
      <p>Erfasst Ausweise/Brevets (z.B. Passagierflugausweis) mit Name und Foto. Ein Foto lässt sich per Kamera aufnehmen oder aus der Galerie wählen; danach kann es per Perspektiv-Korrektur ("✂️") entzerrt werden — vier Eckpunkte auf dem Foto verschieben (Kanten werden automatisch erkannt und vorgeschlagen), das Ergebnis wird gerade zugeschnitten. Fotos lassen sich per Tippen formatfüllend ansehen, einzelne Brevets über "🗑" löschen. Über "+ Weiteres Brevet hinzufügen" lassen sich beliebig viele Einträge anlegen.</p>
    </>),
  },
  {
    id: "backup", title: "7. Datensicherung",
    body: () => (<>
      <p>Über den 💾-Button im Flugbuch oder die Einstellungen kann der komplette Datenbestand (Flüge, Wartung, Ausrüstungsgewichte, Brevets, Reisen, Notizen) als Datei exportiert und auf einem anderen Gerät oder nach einem Neustart wieder eingespielt werden. Regelmässige Backups werden empfohlen.</p>
...
function KurzContent() {
  return (
    <div className="hilfe-body">
      <h3>Einstellungen</h3>
      <p>🪂 Schirme: Kartenmarker wird pro Flug automatisch per Namensabgleich mit dem Schirm-Feld gewählt (z.B. "Mentor" → "Ozone Mentor 7"); ohne Treffer "Eigenes Symbol" (hier einstellbar).</p>
      <h3>Flugbuch</h3>
      <ul>
        <li>Suche: <code>feld:wert</code>, <code>feld=wert</code>, <code>feld&gt;wert</code>, <code>+wort</code> (muss), <code>-wort</code> (darf nicht)</li>
        <li>📥 Import (IGC/CSV) · ☁️ Backup (gzip, roter Punkt = ungesichert) · ☑ Auswahl · 🌐 Weltkarte · 🔍 Suchen/Sortieren ein-/ausblenden</li>
        <li>CSV-Import erkennt Kopfzeilen und ordnet Spalten flexibel zu, unabhängig von Reihenfolge/Sprache</li>
        <li>IGC-Import füllt Zeiten, Ort, Schirm, Höhen (Steigen/Sinken über Druckhöhe + 30s-Gleitmittel) automatisch — Distanz und Routenart (Freie Strecke/Flaches Dreieck/FAI-Dreieck) werden als überschreibbarer Vorschlag vorausgefüllt, massgeblich bleibt der XContest-Wert (Button neben IGC-Badge); bei fehlendem Dateiname-Treffer zusätzlich Datumsabgleich, bei mehreren Kandidaten Nachfrage</li>
        <li>Flugdetail zeigt unter Distanz zusätzlich Max. Freie Strecke/Flaches Dreieck/FAI-Dreieck je mit Punktzahl; Routenart manuell umstellen passt Distanz und Karten-Route automatisch mit an (beides bleibt weiterhin überschreibbar); in der Flugliste erscheint bei bekannter Routenart ein kleines Symbol vor der Distanz (▲ Dreieck, grün bei FAI, ↗ Freie Strecke)</li>
        ...
        <li>💡 Gespeicherte Darstellungen (zwischen Suchen und Sortieren): kompletter Suchen/Sortieren/Gruppieren-Zustand unter einem Namen sicherbar und per Tippen sofort wieder anwendbar; Speichern als… / Verschieben / Löschen; Name der aktiven Darstellung erscheint orange neben der Flug-Anzahl (ausser bei "Standard"), verschwindet bei manueller Änderung</li>
        <li>Statistik-Feld neben der Flug-Anzahl (bei aktiver Suche oder Darstellung): 10 Kennzahlen standardmässig aktiv, 6 weitere (Landeplätze/Flugtage/Biplace-Flüge/Ø Höhengewinn/Max. Steigen/Max. Sinken) über ⚙️ dazuwählbar, alle einzeln aus-/einblendbar und per ↑/↓ neu anordenbar</li>
      ...
      <h3>Statistik</h3>
      <p>7 Badges (Passagiere/Hike blenden sich einzeln aus, wenn nicht zutreffend): Schirm · Startplätze · Landeplätze · Passagiere · Hike (gruppiert nach Ort) · Saison (Jahresübersicht + Rekorde) · Graph (öffnet formatfüllend, passt sich dem Querformat an; Gruppiert: Balken zur Flugliste-Gruppierung, X/Y frei wählbar aus allen Flugdatenfeldern, X-Achse nach eigenem Wert (bei Namen alphabetisch) oder nach Y-Wert sortierbar; Frei: ein Punkt je Flug, gleiche Feldliste, je Achse "Wert" oder "Rang nach der anderen Achse" wählbar für eine sortierte Verteilungskurve, im Zoom Punkt antippen zeigt Flugnummer → antippen öffnet Flugdetail — beide Modi mit optionaler Trendlinie, runden Achsen-Ticks, Gr.-2°-Aufschlüsselung, echtem Bereichs-Zoom per Pinch, Achsen-Feldliste per ⚙️ auswählbar/umsortierbar)</p>
      <h3>Reisen</h3>
      <p>Flüge zu Reisen zusammenfassen, automatische Zuordnung nach Datum möglich</p>
      <h3>Ausrüstung, Brevet</h3>
      <p>Drei Tabs: <b>⚖️ Gewichte</b> — 8 farblich unterschiedene Kategorien (Schirm/Sitz/Reserve/Packhilfen/Geräte/Kleidung/Zubehör/Körpergewicht), Positionen frei hinzufügbar (Name+kg, manuell erfasst), mehrere Setups (z.B. Tandem/Solo) je mit eigener Positions-Auswahl, nur eins gleichzeitig sichtbar, Gesamtgewicht/Limite/Reserve. <b>🛠️ Wartung</b> — 3 Kapitel: Reserve (3) · Schirm (4) · Sitz (5) — Titel per Namens-Liste wählbar (nochmal auf aktiven Titel tippen; bei Schirm mit Flugbuch-Namen vorbefüllt) oder frei eintippbar, Check-Intervall, nächstes fälliges Datum automatisch angezeigt. <b>🎓 Brevet</b> — Ausweise mit Name und Foto, Perspektiv-Korrektur beim Zuschneiden, im Backup enthalten</p>
      <h3>Weltkarte</h3>
