...
function WartungApp() {
    ...
    (async () => {
      try {
        ...
        const PALETTE = { white:"var(--c-text, #e8f4fd)", navy:"#040e20", red:"#f87171", yellow:"#fde047",
          cyan:"#7dd3fc", bordeaux:"#e0304a", orange:"#f5a623", green:"var(--c-accent-wart, #4ade80)", violet:"#c4b5fd" };
        ...
        const CATS = [
          ...
          ["error","--c-error",1], ["rating","--c-rating",1], ["accentWart","--c-accent-wart",1],
        ];
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--c-bg, #051d0e)",color:"var(--c-text, #e8f4fd)",fontFamily:"-apple-system,BlinkMacSystemFont,sans-serif",paddingBottom:40}}>
      {/* Header */}
      ...
      <div style={{padding:"14px 16px 0",display:"flex",gap:10}}>
        <button onClick={()=>{setShowReserve(s=>!s); setShowSchirm(false);}}
          style={{flex:1,background:showReserve?"rgba(34,197,94,0.18)":"rgba(255,255,255,0.05)",border:`1px solid ${showReserve?"rgba(34,197,94,0.4)":"rgba(255,255,255,0.1)"}`,borderRadius:12,padding:"12px 10px",color:showReserve?"var(--c-accent-wart, #4ade80)":"rgba(232,244,253,0.8)",fontSize:14,fontWeight:700,cursor:"pointer",textAlign:"center"}}>
          🪂 Reserve {showReserve?"▾":"▸"}
        </button>
          ...
          <div style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,padding:16,display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div>
              ...
              <select value={schirmData.category} onChange={e=>updateSchirmSlot(activeSchirmSlot,{category:e.target.value})}
                style={{width:"100%",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"9px 10px",color:"var(--c-text, #e8f4fd)",fontSize:14,boxSizing:"border-box"}}>
                {SCHIRM_CATEGORY_OPTIONS.map(opt => (
                  <option key={opt} value={opt} style={{background:"#0d1b2a",color:"var(--c-text, #e8f4fd)"}}>{opt}</option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <input type="number" min="1" value={schirmData.intervalMonths}
                  ...
                  style={{width:70,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"9px 10px",color:"var(--c-text, #e8f4fd)",fontSize:14,boxSizing:"border-box"}} />
            ...
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                ...
                <span style={{fontSize:12,fontWeight:700,padding:"3px 9px",borderRadius:20,
                  ...
                  color: schirmOverdue ? "#f87171" : schirmSoonDue ? "#fcd34d" : "var(--c-accent-wart, #4ade80)"}}>
                  {schirmOverdue ? "Überfällig" : `Nächster Check ${fmtDate(schirmNextDue)}`}
                </span>
                <button onClick={()=>addSchirmCheck(activeSchirmSlot, todayStr())}
                  style={{background:"rgba(34,197,94,0.15)",border:"1px solid rgba(34,197,94,0.3)",borderRadius:20,padding:"4px 10px",color:"var(--c-accent-wart, #4ade80)",fontSize:11,fontWeight:700,cursor:"pointer"}}>
                  + Check
                </button>
              </div>
                ...
                {(schirmData.checks||[]).map((c, idx) => (
                  <div key={idx} style={{display:"flex",gap:8,alignItems:"center"}}>
                    <input value={c.note} onChange={e=>updateSchirmCheck(activeSchirmSlot, idx, {note:e.target.value})}
                      ...
                      style={{flex:1,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"8px 10px",color:"var(--c-text, #e8f4fd)",fontSize:13,boxSizing:"border-box"}} />
                    <input value={c.date} onChange={e=>updateSchirmCheck(activeSchirmSlot, idx, {date:e.target.value})}
                      placeholder="TT.MM.JJJJ"
                      style={{width:110,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"8px 10px",color:"var(--c-text, #e8f4fd)",fontSize:13,boxSizing:"border-box"}} />
            ...
            {RESERVE_SLOTS.map(slot => (
              <button key={slot.id} onClick={()=>setActiveReserveSlot(slot.id)}
                style={{
                  ...
                  color: activeReserveSlot===slot.id ? "var(--c-accent-wart, #4ade80)" : "rgba(232,244,253,0.5)",
                }}>
                {slot.label}
              </button>
            ))}
            ...
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                ...
                <span style={{fontSize:12,fontWeight:700,padding:"3px 9px",borderRadius:20,
                  ...
                  color: overdue ? "#f87171" : soonDue ? "#fcd34d" : "var(--c-accent-wart, #4ade80)"}}>
