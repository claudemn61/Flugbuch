...
const VERSION_LOG = [
  { v: "2.8", note: "Neu: Farbschema für alle 4 Seiten (Einstellungen → 🎨 Farben) — Liste von Verwendungs-Kategorien (Hintergrund, Text, Rahmen, Akzentfarben je Seite, u.a.), jede mit Auswahl aus den bereits im Design verwendeten Farben (kein freier Picker, nur Umverteilung bestehender Farben). Gespeichert, im Backup enthalten." },
...
// The "small table" of colours already used somewhere across the app's 4
// pages — the person can only reassign which of THESE gets used for which
// purpose, not introduce arbitrary new colours. hex is the opaque swatch
// shown in the picker; each category below pairs a colour with a fixed
// alpha appropriate to how it's actually used (e.g. muted text keeps its
// usual low opacity regardless of which colour is picked for it).
const PALETTE_COLORS = [
  { key: "white",    label: "Weiss (Text)",           hex: "#e8f4fd" },
  { key: "navy",     label: "Dunkelblau (Hintergrund)", hex: "#040e20" },
  { key: "red",      label: "Rot (Fehler)",            hex: "#f87171" },
  { key: "yellow",   label: "Gelb (Bewertung)",        hex: "#fde047" },
  { key: "cyan",     label: "Cyan (Flugbuch-Akzent)",  hex: "#7dd3fc" },
  { key: "bordeaux", label: "Bordeaux (Statistik-Akzent)", hex: "#e0304a" },
  { key: "orange",   label: "Orange (Reisen-Akzent)",  hex: "#f5a623" },
  { key: "green",    label: "Grün (Wartung-Akzent)",   hex: "#4ade80" },
  { key: "violet",   label: "Violett (CSV-Kennzeichnung)", hex: "#c4b5fd" },
];
// Each category: which CSS variable it sets, its usual alpha, and which
// palette colour it defaults to (matching what the app already looks like
// today, so nothing changes visually until the person reassigns something).
const COLOR_CATEGORIES = [
  { key: "bg",           varName: "--c-bg",             label: "Hintergrund (alle Seiten)",      alpha: 1,    default: "navy" },
  { key: "card",         varName: "--c-card",           label: "Kartenhintergrund (alle Seiten)", alpha: 0.05, default: "white" },
  { key: "text",         varName: "--c-text",           label: "Haupttext (alle Seiten)",        alpha: 1,    default: "white" },
  { key: "textMuted",    varName: "--c-text-muted",     label: "Gedämpfter Text (alle Seiten)",  alpha: 0.4,  default: "white" },
  { key: "border",       varName: "--c-border",         label: "Rahmenfarbe (alle Seiten)",      alpha: 0.1,  default: "white" },
  { key: "error",        varName: "--c-error",          label: "Fehlerfarbe (alle Seiten)",      alpha: 1,    default: "red" },
  { key: "rating",       varName: "--c-rating",         label: "Bewertung/Warnfarbe (alle Seiten)", alpha: 1, default: "yellow" },
  { key: "accentFlug",   varName: "--c-accent-flug",    label: "Akzentfarbe: Flugbuch",          alpha: 1,    default: "cyan" },
  { key: "accentStat",   varName: "--c-accent-stat",    label: "Akzentfarbe: Statistik",         alpha: 1,    default: "bordeaux" },
  { key: "accentReisen", varName: "--c-accent-reisen",  label: "Akzentfarbe: Reisen",            alpha: 1,    default: "orange" },
  { key: "accentWart",   varName: "--c-accent-wart",    label: "Akzentfarbe: Wartung",           alpha: 1,    default: "green" },
];
...
function defaultColorMap() {
  const m = {};
  for (const c of COLOR_CATEGORIES) m[c.key] = c.default;
  return m;
}
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (r) setColorMap({ ...defaultColorMap(), ...JSON.parse(r.value) });
      } catch {}
    })();
  }, []);
  const saveColorMap = async (next) => {
    setColorMap(next);
    setColorStatus("saving");
    try {
      await window.storage.set("settings:colorMap", JSON.stringify(next));
      setColorStatus("saved");
      setTimeout(()=>setColorStatus(""), 1200);
    } catch { setColorStatus("error"); }
  };
  const resetColorMap = () => saveColorMap(defaultColorMap());
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:300,display:"flex",alignItems:"flex-end",justifyContent:"center"}}
      ...
      <div onClick={e=>e.stopPropagation()}
        ...
        {/* Farben: Verwendungs-Kategorien, jede mit einer Auswahlliste aus
            den bereits im Design verwendeten Farben (kein freier Picker —
            nur Umverteilung bestehender Farben). Gilt für alle 4 Seiten. */}
        <div style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,overflow:"hidden",marginTop:10}}>
          <div onClick={()=>setOpenFolder(openFolder==="colors"?null:"colors")}
            style={{padding:"12px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer"}}>
            <span style={{fontSize:14,fontWeight:700,color:"#e8f4fd"}}>🎨 Farben</span>
            <span style={{color:"rgba(232,244,253,0.4)",fontSize:13}}>{openFolder==="colors"?"▾":"▸"}</span>
          </div>
          {openFolder==="colors" && (
            <div style={{padding:"0 14px 14px"}}>
              <div style={{fontSize:11,color:"rgba(232,244,253,0.4)",marginBottom:10}}>
                Jede Zeile zeigt, wofür eine Farbe aktuell verwendet wird — die Auswahl daneben nutzt ausschliesslich Farben, die bereits irgendwo im Design vorkommen.
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {COLOR_CATEGORIES.map(cat => {
                  const chosen = PALETTE_COLORS.find(p=>p.key===colorMap[cat.key]) || PALETTE_COLORS.find(p=>p.key===cat.default);
                  return (
                    <div key={cat.key} style={{display:"flex",alignItems:"center",gap:10,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:10,padding:"8px 10px"}}>
                      <div style={{width:20,height:20,borderRadius:5,background:hexToRgba(chosen.hex, cat.alpha<1?1:cat.alpha),border:"1px solid rgba(255,255,255,0.25)",flexShrink:0}} />
                      <span style={{flex:1,fontSize:12,color:"#e8f4fd"}}>{cat.label}</span>
                      <select value={colorMap[cat.key]||cat.default}
                        onChange={e=>saveColorMap({...colorMap,[cat.key]:e.target.value})}
                        style={{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.15)",borderRadius:8,padding:"5px 6px",color:"#e8f4fd",fontSize:11}}>
                        {PALETTE_COLORS.map(p=>(
                          <option key={p.key} value={p.key} style={{background:"#0a1628"}}>{p.label}</option>
                        ))}
                      </select>
                    </div>
                  );
              ...
              <div style={{display:"flex",alignItems:"center",gap:10,marginTop:12}}>
                <button onClick={resetColorMap}
                  style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:10,padding:"8px 14px",color:"rgba(232,244,253,0.7)",fontSize:12,cursor:"pointer"}}>
                  ↺ Zurücksetzen
                </button>
                {colorStatus==="saved" && <span style={{fontSize:12,color:"#4ade80"}}>✓ Gespeichert</span>}
                {colorStatus==="error" && <span style={{fontSize:12,color:"#f87171"}}>Fehler beim Speichern</span>}
              </div>
              <div style={{fontSize:11,color:"rgba(232,244,253,0.35)",marginTop:8}}>Änderungen wirken nach dem nächsten Neuladen der jeweiligen Seite.</div>
            </div>
          )}
        </div>
      </div>
