...
const VERSION_LOG = [
  { v: "2.7", note: "Neu: Farbschema für die Flugbuch-Seite frei editierbar (Einstellungen → 🎨 Farben) — 5 Rollen (Hintergrund, Akzent, Text Haupt/gedämpft, Kartenfarbe), Auswahl über ein iOS-artiges 10×12-Raster inkl. Graustufen-Reihe plus Deckkraft-Regler. Gespeichert und im Backup enthalten. Statistik/Reisen/Wartung folgen in einem späteren Update." },
...
// iOS-style colour picker: 12 hue columns × 9 lightness rows, plus a 10th
// row of pure grayscale (12 steps white→black), and an opacity slider.
// Colours are kept as {h,s,l,a} rather than a flat string so the swatch
// grid can highlight the currently selected cell and the opacity slider
// can adjust alpha independently of which swatch is picked.
function ColorPicker({ value, onChange }) {
  const rows = React.useMemo(() => {
    ...
    for (let r = 0; r < 9; r++) {
      const l = 88 - r*9;
      const row = [];
      for (let c = 0; c < 12; c++) row.push({ h: c*30, s: 75, l });
      out.push(row);
    }
    const grayRow = [];
    for (let c = 0; c < 12; c++) grayRow.push({ h: 0, s: 0, l: Math.round(c*(100/11)) });
    out.push(grayRow);
    return out;
  }, []);
  const v = value || { h: 0, s: 0, l: 50, a: 1 };
  return (
    <div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(12, 1fr)",gap:3,marginBottom:10}}>
        {rows.flat().map((sw, i) => {
          const selected = v.h===sw.h && v.s===sw.s && v.l===sw.l;
          return (
            <div key={i} onClick={()=>onChange({...sw, a: v.a})}
              style={{aspectRatio:"1",borderRadius:4,background:`hsl(${sw.h},${sw.s}%,${sw.l}%)`,border:selected?"2px solid #fff":"1px solid rgba(255,255,255,0.15)",cursor:"pointer",boxShadow:selected?"0 0 0 1px rgba(0,0,0,0.4)":"none"}} />
          );
        })}
      </div>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <div style={{width:34,height:34,borderRadius:8,background:`hsla(${v.h},${v.s}%,${v.l}%,${v.a})`,border:"1px solid rgba(255,255,255,0.25)",flexShrink:0,backgroundImage:"repeating-conic-gradient(#8883 0% 25%, transparent 0% 50%)",backgroundSize:"10px 10px"}}>
          <div style={{width:"100%",height:"100%",borderRadius:7,background:`hsla(${v.h},${v.s}%,${v.l}%,${v.a})`}} />
        </div>
        <input type="range" min="0" max="100" value={Math.round((v.a??1)*100)}
          onChange={e=>onChange({...v, a: (+e.target.value)/100})}
          style={{flex:1}} />
        <span style={{fontSize:11,color:"rgba(232,244,253,0.6)",width:38,textAlign:"right",flexShrink:0}}>{Math.round((v.a??1)*100)}%</span>
      </div>
    </div>
  );
}

// Colour roles for a page's theme, and the Flugbuch default (matches the
// colours the app already shipped with, converted to HSL).
const THEME_ROLES = [
  { key: "background", label: "Hintergrund" },
  { key: "accent",     label: "Akzentfarbe" },
  { key: "textMain",   label: "Text (Haupt)" },
  { key: "textMuted",  label: "Text (gedämpft)" },
  { key: "card",       label: "Kartenfarbe" },
];
const DEFAULT_THEME_FLUGBUCH = {
  background: { h:219, s:78, l:7,  a:1 },
  accent:     { h:199, s:95, l:74, a:1 },
  textMain:   { h:206, s:84, l:95, a:1 },
  textMuted:  { h:206, s:84, l:95, a:0.4 },
  card:       { h:0,   s:0,  l:100, a:0.05 },
};
function themeToCss(role) {
  return `hsla(${role.h},${role.s}%,${role.l}%,${role.a})`;
}
...
function SettingsOverlay({ onClose }) {
  ...
  const [theme, setTheme] = useState(DEFAULT_THEME_FLUGBUCH);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (r) setTheme({ ...DEFAULT_THEME_FLUGBUCH, ...JSON.parse(r.value) });
      } catch {}
    })();
  }, []);
  const saveTheme = async (next) => {
    ...
    try {
      await window.storage.set("settings:theme:flugbuch", JSON.stringify(next));
      setThemeStatus("saved");
      setTimeout(()=>setThemeStatus(""), 1200);
    } catch { setThemeStatus("error"); }
  };
  const resetTheme = () => saveTheme(DEFAULT_THEME_FLUGBUCH);
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:300,display:"flex",alignItems:"flex-end",justifyContent:"center"}}
      ...
      <div onClick={e=>e.stopPropagation()}
        ...
        {/* Farben: pro Seite editierbares Farbschema (bisher nur Flugbuch —
            Statistik/Reisen/Wartung folgen in einem späteren Schritt). */}
          ...
          {openFolder==="colors" && (
            <div style={{padding:"0 14px 14px"}}>
              <div style={{display:"flex",gap:6,marginBottom:12,overflowX:"auto"}}>
                <span style={{background:"rgba(125,211,252,0.18)",border:"1px solid rgba(125,211,252,0.4)",borderRadius:20,padding:"6px 12px",color:"#7dd3fc",fontSize:12,fontWeight:700,whiteSpace:"nowrap"}}>Flugbuch</span>
                {["Statistik","Reisen","Wartung"].map(p=>(
                  <span key={p} style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:20,padding:"6px 12px",color:"rgba(232,244,253,0.3)",fontSize:12,whiteSpace:"nowrap"}}>{p} (bald)</span>
                ))}
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {THEME_ROLES.map(role => (
                  <div key={role.key}>
                    <button onClick={()=>setEditingRole(editingRole===role.key?null:role.key)}
                      style={{width:"100%",display:"flex",alignItems:"center",gap:10,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:10,padding:"9px 12px",cursor:"pointer"}}>
                      <div style={{width:22,height:22,borderRadius:6,background:themeToCss(theme[role.key]),border:"1px solid rgba(255,255,255,0.25)",flexShrink:0}} />
                      <span style={{flex:1,textAlign:"left",color:"#e8f4fd",fontSize:13}}>{role.label}</span>
                      <span style={{color:"rgba(232,244,253,0.4)",fontSize:12}}>{editingRole===role.key?"▾":"▸"}</span>
                    </button>
                    {editingRole===role.key && (
                      <div style={{padding:"10px 4px 4px"}}>
                        <ColorPicker value={theme[role.key]} onChange={v=>saveTheme({...theme,[role.key]:v})} />
                      </div>
                    )}
                  </div>
                ))}
              ...
              <div style={{display:"flex",alignItems:"center",gap:10,marginTop:12}}>
                ...
                {themeStatus==="saved" && <span style={{fontSize:12,color:"#4ade80"}}>✓ Gespeichert</span>}
                {themeStatus==="error" && <span style={{fontSize:12,color:"#f87171"}}>Fehler beim Speichern</span>}
              </div>
              <div style={{fontSize:11,color:"rgba(232,244,253,0.35)",marginTop:8}}>Änderungen wirken nach dem nächsten Neuladen der Flugbuch-Seite.</div>
            </div>
          )}
        </div>
      </div>
