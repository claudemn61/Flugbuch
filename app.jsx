...
function DetailContent({ fl, flights, customFieldDefs, setFlights, setSelected, setView, setInlinePassagier, setEditData, saveFlight, showFieldEditor, setShowFieldEditor, handleSaveFields, confirmDelete, setConfirmDelete, hideBackButton }) {
...
function SidebarList({ flights, selectedId, onSelect, longestId }) {
  ...
  return (
    <div style={{width:340,minWidth:340,height:"100vh",overflowY:"auto",borderRight:"1px solid rgba(255,255,255,0.08)",background:"#0a1628"}}>
      <div style={{padding:"14px 14px 8px",position:"sticky",top:0,background:"#0a1628",zIndex:5,borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
        <input value={filterText} onChange={e=>setFilterText(e.target.value)} placeholder="🔍 Suchen…"
          style={{width:"100%",background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,padding:"8px 12px",color:"#e8f4fd",fontSize:13,boxSizing:"border-box"}} />
      </div>
      {years.map(yr => {
        const yFlights = filtered.filter(f=>f.year===yr);
        return (
          <div key={yr}>
