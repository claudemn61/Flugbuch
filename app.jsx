...
function SettingsOverlay({ onClose }) {
  ...
  const saveNotes = async (v) => {
    setNotes(v);
    try { await window.storage.set("settings:notes", v); } catch {}
  };
      ...
      <div onClick={e=>e.stopPropagation()}
        ...
        <div style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,overflow:"hidden"}}>
          <div onClick={()=>setOpenFolder(openFolder==="notes"?null:"notes")}
            style={{padding:"12px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer"}}>
            <span style={{fontSize:14,fontWeight:700,color:"#e8f4fd"}}>📝 Notizen</span>
    ...
    <div style={{
      ...
      <div style={{ padding: "10px 20px", display: "flex", flexDirection: "column", gap: 9, flex: 1, minHeight: 0 }}>
        {TILES.map((t) => (
          <a
            key={t.id}
            href={t.ready && t.href ? t.href : undefined}
            onClick={(e) => {
              if (!(t.ready && t.href)) e.preventDefault();
            }}
            style={{
              position: "relative",
              display: "flex",
