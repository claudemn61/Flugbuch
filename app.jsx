...
// Builds one 53-column CSV/TSV row, but ONLY fills the columns the person
// explicitly wants overwritten (their Numbers sheet has formulas in the rest,
// which must stay untouched): Datum, Startzeit, Landezeit, Distanz, müM S,
// müM. L, H.Max, H.Gew., Sinken (⬇️), Steigen (⬆️), Gerät, Passagier,
// Bemerkung. Start and Landung are formula columns in the person's sheet and
// are deliberately left blank. Every other column is left as an empty
// string so a paste into Numbers doesn't clobber formula cells.
function flightToCsvRow(f) {
  const cf = f.customFields || {};
  const cols = new Array(53).fill("");
  cols[5]  = f.rawDate || f.date || "";                                    // Datum
  cols[6]  = f.startTime || "";                                            // Startzeit
  cols[20] = f.endTime || "";                                              // Landezeit
  cols[37] = f.totalDist ? String(f.totalDist) : (cf.distKm || "");        // Distanz
  cols[40] = f.startAlt ? String(f.startAlt) : (cf.msa || "");             // müM S
  cols[41] = f.endAlt ? String(f.endAlt) : (cf.ml || "");                  // müM. L
  cols[42] = f.maxAlt ? String(f.maxAlt) : (cf.hMax || "");                // H.Max
  cols[44] = cf.hGew || "";                                                // H.Gew.
  cols[45] = cf.maxSinken || "";                                           // ⬇️
  cols[46] = cf.maxSteigen || "";                                          // ⬆️
  cols[47] = f.glider || "";                                               // Gerät
  cols[48] = cf.passagier || "";                                           // Passagier
  cols[52] = f.notes || "";                                                // Bemerkung
  return cols.join("\t");
}
        ...
        {selectMode && (
          <button onClick={async()=>{
              ...
              try {
                ...
                const cellStyle = "font-family:Helvetica;font-size:10pt;";
