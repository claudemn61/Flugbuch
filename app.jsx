...
function flightToCsvRow(f) {
  ...
  const cols = new Array(53).fill("");
  cols[0] = f.name || "";
  cols[5] = f.rawDate || f.date || "";
  cols[6] = f.startTime || "";
  cols[10] = f.site || "";
  if (f.startPt) {
    const c = wgs84ToLv03(f.startPt.lat, f.startPt.lon);
    cols[12] = String(c.e); cols[13] = String(c.n);
  }
  cols[20] = f.endTime || "";
  cols[23] = cf.landung || "";
  if (f.endPt) {
    const c = wgs84ToLv03(f.endPt.lat, f.endPt.lon);
    cols[25] = String(c.e); cols[26] = String(c.n);
  }
  cols[34] = f.durationStr || "";
  cols[37] = f.totalDist ? String(f.totalDist) : (cf.distKm || "");
  cols[38] = cf.kmh || "";
  cols[39] = cf.hDiff || "";
  cols[40] = f.startAlt ? String(f.startAlt) : (cf.msa || "");
  cols[41] = f.endAlt ? String(f.endAlt) : (cf.ml || "");
  cols[42] = f.maxAlt ? String(f.maxAlt) : (cf.hMax || "");
  cols[44] = cf.hGew || "";
  cols[45] = cf.maxSinken || "";
  cols[46] = cf.maxSteigen || "";
  cols[47] = f.glider || "";
  cols[48] = cf.passagier || "";
  cols[52] = f.notes || "";
  return cols.join("\t");
}
...
function sortFieldValue(f, sortId) {
  const cf = f.customFields || {};
  switch (sortId) {
    case "date":     return parseDateToTs(f.date || f.rawDate);
    ...
    <div style={{maxWidth:isWide?900:480,margin:"0 auto",minHeight:"100vh",background:"#0a1628",color:"#e8f4fd",fontFamily:"-apple-system,BlinkMacSystemFont,sans-serif"}}>
      ...
      <div style={{padding:"8px 16px 0",display:"flex",gap:8,alignItems:"center"}}>
        <button onClick={exportBackup}
          style={{flex:1,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,padding:"7px 10px",color:"rgba(232,244,253,0.7)",fontSize:11,cursor:"pointer"}}>
          ☁️ In iCloud sichern
        </button>
      ...
      <div style={{padding:"4px 16px 0",fontSize:10,color:"rgba(232,244,253,0.35)"}}>
        Tipp: Im Freigabe-Menü "In Dateien sichern" wählen → iCloud Drive als Ziel.
      </div>
      {backupMsg && (
        <div style={{padding:"6px 16px 0",fontSize:11,color:backupMsg.startsWith("✓")?"#4ade80":"#f87171"}}>
          {backupMsg}
        </div>
      )}
      ...
      <div style={{padding:"8px 16px 0",display:"flex",gap:8,alignItems:"center"}}>
        <button onClick={()=>{ setSelectMode(m=>!m); setSelectedIds(new Set()); setCopyMsg(""); }}
          style={{background:selectMode?"rgba(14,165,233,0.18)":"rgba(255,255,255,0.05)",border:`1px solid ${selectMode?"rgba(14,165,233,0.4)":"rgba(255,255,255,0.1)"}`,borderRadius:10,padding:"7px 12px",color:selectMode?"#7dd3fc":"rgba(232,244,253,0.7)",fontSize:11,cursor:"pointer"}}>
          {selectMode?"✕ Auswahl beenden":"☑ Flüge auswählen"}
        </button>
        {selectMode && (
          <button onClick={async()=>{
              ...
              try {
                ...
                setCopyMsg(`✓ ${chosen.length} Flug${chosen.length!==1?"e":""} kopiert — in Numbers einfügen.`);
              } catch (e) {
                setCopyMsg("Fehler beim Kopieren: " + e.message);
              }
            }}
            style={{flex:1,background:"rgba(34,197,94,0.15)",border:"1px solid rgba(34,197,94,0.3)",borderRadius:10,padding:"7px 12px",color:"#4ade80",fontSize:11,fontWeight:700,cursor:"pointer"}}>
            📋 {selectedIds.size} kopieren
          </button>
        )}
      </div>
      {copyMsg && (
      ...
      <div style={{padding:"0 16px 10px",position:"relative"}}>
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>setShowSortMenu(s=>!s)}
            ...
            <span>⇅ Sortieren: {SORT_OPTIONS.find(o=>o.id===sortId)?.label||"—"}</span>
            <span>{showSortMenu?"▾":"▸"}</span>
          </button>
          <button onClick={()=>setSortDir(d=>d==="asc"?"desc":"asc")}
            title={sortDir==="asc"?"Aufsteigend":"Absteigend"}
            style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,padding:"8px 14px",color:"#7dd3fc",fontSize:14,cursor:"pointer"}}>
            {sortDir==="asc"?"↑":"↓"}
          </button>
        </div>
        {showSortMenu && (
      ...
      <div style={{margin:"0 16px 10px"}}>
        <button onClick={()=>setShowRowImport(s=>!s)}
          ...
          <span>📝 Zeile(n) einfügen (Numbers/CSV)</span>
      ...
      <div style={{padding:"4px 0 16px"}}>
        ...
        {sortId !== "date" ? (
          ...
          <div style={isWide?{display:"grid",gridTemplateColumns:"1fr 1fr",gap:0}:{}}>
            {sortFlights([...filteredFlights, ...noYear.filter(f=>!filteredFlights.includes(f))], sortId, sortDir).map(f=>(
        ...
        {years.map(yr => {
          ...
          return (
            <div key={yr}>
              ...
              {!collapsed && <div style={isWide?{display:"grid",gridTemplateColumns:"1fr 1fr",gap:0}:{}}>
              {yFlights.map(f=>(
                <FlightRow key={f.id} f={f} isLongest={f.id===longestId} sortId={sortId}
                  selectMode={selectMode} isSelected={selectedIds.has(f.id)}
                  onToggleSelect={id=>setSelectedIds(prev=>{const n=new Set(prev);n.has(id)?n.delete(id):n.add(id);return n;})}
