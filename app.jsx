...
function FlightRow({ f, isLongest, onClick }) {
...
function FlugbuchApp() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        console.log("[DEBUG] storage.list returned keys:", keys?.keys?.length, keys?.keys);
        const loaded = await Promise.all((keys?.keys||[]).map(async k => {
          try { const r = await window.storage.get(k); return r ? JSON.parse(r.value) : null; } catch { return null; }
        }));
        console.log("[DEBUG] loaded (non-null):", loaded.filter(Boolean).length);
  ...
  return (
    <div style={{maxWidth:isWide?900:480,margin:"0 auto",minHeight:"100vh",background:"#0a1628",color:"#e8f4fd",fontFamily:"-apple-system,BlinkMacSystemFont,sans-serif"}}>
      ...
      <div style={{padding:"6px 16px 0",fontSize:10,color:"#fcd34d",fontFamily:"monospace"}}>
        DEBUG: {flights.length} Flüge im React-State geladen
      </div>
          ...
          <div style={{marginTop:6,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:10,padding:10}}>
            ...
            <button onClick={()=>{
                ...
                try {
                  ...
                  let newNr;
                  const parsedNr = parseInt((p._nr||"").match(/\d+/)?.[0]||"",10);
                  if (parsedNr) {
                    newNr = String(parsedNr);
                  } else {
                    const maxNr = flights.reduce((m,f)=>{
                      const n = parseInt((f.name||"").match(/\d+/)?.[0]||"0",10);
                      return n>m?n:m;
                    }, 0);
                    newNr = String(maxNr+1);
                  }
