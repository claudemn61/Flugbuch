...
function ServiceApp() {
  ...
  const updateSchirmCheck = (slotId, idx, patch) => {
    const slot = schirme[slotId];
    const checks = slot.checks.map((c,i) => i===idx ? {...c, ...patch} : c);
    updateSchirmSlot(slotId, { checks });
  };
  ...
  const schirmLastCheck = schirmData.checks && schirmData.checks.length ? parseDateStr(schirmData.checks[0].date) : null;
            ...
            {SCHIRM_SLOT_IDS.map(slotId => {
              ...
              return (
                <button key={slotId} onClick={()=>setActiveSchirmSlot(slotId)}
                  ...
                  {slot.category === "–" ? "– leer –" : slot.category}
                </button>
              );
            })}
          </div>
          ...
          <div style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,padding:16,display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div>
              ...
              <div style={{display:"flex",gap:8}}>
                ...
                <button onClick={()=>updateSchirmSlot(activeSchirmSlot,{purchaseDate:todayStr()})}
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                ...
                {schirmNextDue && (
                  <div style={{fontSize:12,fontWeight:700,color:"#fcd34d"}}>
                    Nächster Check {fmtDate(schirmNextDue)}
                  </div>
                )}
          ...
          <div style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,padding:16,display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div>
              ...
              <div style={{display:"flex",gap:8}}>
                <input value={data.purchaseDate} onChange={e=>updateSlot(activeReserveSlot,{purchaseDate:e.target.value})}
                  placeholder="TT.MM.JJJJ"
                  style={{flex:1,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"9px 10px",color:"#e8f4fd",fontSize:14,boxSizing:"border-box"}} />
                <button onClick={()=>updateSlot(activeReserveSlot,{purchaseDate:todayStr()})}
                  style={{background:"rgba(125,211,252,0.12)",border:"1px solid rgba(125,211,252,0.25)",borderRadius:8,padding:"0 12px",color:"#7dd3fc",fontSize:12,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap"}}>
                  heute
                </button>
              </div>
            ...
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                ...
                {nextDue && (
                  <div style={{fontSize:12,fontWeight:700,color:"#fcd34d"}}>
                    Nächster Check {fmtDate(nextDue)}
                  </div>
                )}
